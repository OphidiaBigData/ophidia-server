...
{
	if (!list) return OPH_JOB_LIST_ERROR;
...
oph_job_info* oph_find_job_in_job_list(oph_job_list* list, int jobid, oph_job_info** prev)
{
	...
	for (temp = list->head; temp; temp_prev = temp, temp = next)
	{
		...
		if (temp->timestamp < remote_deadline)
		{
			...
			if (list->tail == temp) list->tail = temp_prev;
			if (list->head == temp) list->head = next;
			else if (temp_prev) temp_prev->next = next;
			list->counter--;
			...
			else oph_workflow_free(temp->wf);

			free(temp);
		}
		else if (temp->wf->idjob == jobid)
		{
			if (prev) *prev = temp_prev;
			return temp;
		}
	}
	return NULL;
...
int oph_insert_into_job_list(oph_job_list* list, oph_job_info* item)
{
	...
	{
		if (oph_server_queue_size && (list->counter >= oph_server_farm_size + oph_server_queue_size)) return OPH_JOB_LIST_FULL;
		result = OPH_JOB_LIST_FARM_FULL;
	}
	if (oph_find_job_in_job_list(list, item->wf->idjob, 0)) return OPH_JOB_LIST_EXISTING_JOB;
...
int oph_drop_from_job_list(oph_job_list* list, oph_job_info* item, oph_job_info* prev)
{
	if (!list) return OPH_JOB_LIST_ERROR;
	list->counter--;
	if (list->tail == item) list->tail = prev;
	if (list->head == item) list->head = item->next;
	else
	{
		if (!prev) return OPH_JOB_LIST_ERROR;
		prev->next = item->next;
	}
	return OPH_JOB_LIST_OK;
}
...
{
	if (oph_drop_from_job_list(list, item, prev)) return OPH_JOB_LIST_ERROR;
...
{
	if (!list) return OPH_JOB_LIST_ERROR;
	if (oph_free_job_list(list)) return OPH_JOB_LIST_ERROR;
	free(list);
	return OPH_JOB_LIST_OK;
}
...
oph_job_info* oph_find_workflow_in_job_list_to_drop(oph_job_list* list, const char* sessionid, int workflowid, oph_job_info** prev)
{
	...
	for (temp = list->head; temp; temp_prev = temp, temp = next)
	{
		...
		else if ( (temp->wf->workflowid == workflowid) && (!sessionid || !strcmp(temp->wf->sessionid,sessionid)) ) return temp;
		if (prev) *prev = temp;
	}
	return NULL;
}
...
oph_job_info* oph_find_marker_in_job_list(oph_job_list* list, const char* sessionid, int markerid, int* task_index, int* light_task_index)
{
	...
	if (light_task_index) *light_task_index = -1;
	...
	for (temp = list->head; temp; temp_prev = temp, temp = next)
	{
		...
		else if ( (temp->wf->markerid == markerid) && (!sessionid || !strcmp(temp->wf->sessionid,sessionid)) ) return temp;
		else for (i=0; i<temp->wf->tasks_num;++i)
		{
			if ( (temp->wf->tasks[i].markerid == markerid) && (!sessionid || !strcmp(temp->wf->sessionid,sessionid)) )
			{
				if (task_index) *task_index = i;
				return temp;
			}
			else for (j=0;j<temp->wf->tasks[i].light_tasks_num;++j) if ( (temp->wf->tasks[i].light_tasks[j].markerid == markerid) && (!sessionid || !strcmp(temp->wf->sessionid,sessionid)) )
			{
				if (task_index) *task_index = i;
				if (light_task_index) *light_task_index = j;
				return temp;
			}
		}
	}
...
oph_job_info* oph_find_unstarted_in_job_list(oph_job_list* list)
{
	...
	for (temp = list->head; temp; temp_prev = temp, temp = next)
	{
		...
		else if (temp->wf->status) number_of_running_wf++;
	}
	...
	if (oph_server_farm_size && (number_of_running_wf < oph_server_farm_size) && (list->counter > number_of_running_wf))
		for (temp = list->head; temp; temp = temp->next) // Find a workflow in queue
			if (!temp->wf->status) return temp; // not started yet
	return NULL;
}
