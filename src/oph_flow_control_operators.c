...
#define OPH_FS_COMMAND "operator=oph_fs;command=ls;file=%s;sessionid=%s;workflowid=%d;markerid=%s;taskindex=%d;lighttaskindex=-1;username=%s;userid=%d;userrole=%d;parentid=%d;"
...
void *_oph_wait(oph_notify_data * data)
{
	...
	if (success && !is_http && (wd->type == 'f')) {
		...
		snprintf(command, OPH_MAX_STRING_SIZE, OPH_FS_COMMAND "%s%s" OPH_SERVER_REQUEST_FLAG, pointer, sessionid, wf->workflowid, str_markerid, task_index, wf->username, wf->iduser,
			 wf->userrole, wf->idjob, wd->measure ? measure : "", wd->subset_params ? wd->subset_params : "");
	} else
		*command = 0;
...
int oph_set_impl(oph_workflow * wf, int i, char *error_message, struct oph_plugin_data *state, char has_action)
{
	*error_message = 0;
	...
	char *arg_value, *error_msg = NULL, *taskname = NULL, first = 1, repeat = 0, compress_value = 0, got_array = 0;
	oph_workflow *twf = wf;
	...
	while (!success) {
		...
		for (j = 0; j < wf->tasks[i].arguments_num; ++j) {
			...
			if (!got_array && !strcasecmp(wf->tasks[i].arguments_keys[j], OPH_ARG_KEY) && strcmp(wf->tasks[i].arguments_values[j], OPH_COMMON_NULL)) {
