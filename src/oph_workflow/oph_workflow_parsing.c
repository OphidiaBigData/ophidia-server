...
int oph_workflow_load(char *json_string, const char *username, oph_workflow **workflow) {
	...
	*workflow = NULL;
	...
	if (run && strlen(run)) {
		if (!strcmp(run,OPH_WORKFLOW_NO)) (*workflow)->run = 0;
	...
	for (i = 0; i < (*workflow)->tasks_num; i++) {
			...
			if ((*workflow)->tasks[i].deps_num >= 1) {
				...
				for (j = 0; j < (*workflow)->tasks[i].deps_num; j++) {
					...
					if (dependency) {
						...
						json_unpack(dependency,"{s?s,s?s,s?s,s?s,s?s,s?s,s?s}","argument",&argument,"order",&order,"task",&task_name,"type",&type,"filter",&filter,"output_argument",&output_argument,"output_order",&output_order);
		...
		if (!on_error_task) on_error_task = on_error;
		...
		if (on_error_task)
		{
			if (!strcmp(on_error_task,OPH_WORKFLOW_SKIP)) (*workflow)->tasks[i].retry_num = -1;
			else if (!strcmp(on_error_task,OPH_WORKFLOW_CONTINUE)) (*workflow)->tasks[i].retry_num = 0;
			else if (!strcmp(on_error_task,OPH_WORKFLOW_BREAK)) (*workflow)->tasks[i].retry_num = 1;
		...
		else (*workflow)->tasks[i].retry_num = 1; // Default value
		(*workflow)->tasks[i].residual_retry_num = (*workflow)->tasks[i].retry_num;

		// Set the exit code
		if (!on_exit_task) on_exit_task = on_exit;
		...
		if (on_exit_task)
		{
			if (!strlen(on_exit_task) || !strcmp(on_exit_task,OPH_WORKFLOW_NOP)) (*workflow)->tasks[i].exit_action = 0;
			else if (!strcmp(on_exit_task,OPH_WORKFLOW_DELETE)) (*workflow)->tasks[i].exit_action = -1;
			else
			{
				oph_workflow_free(*workflow);
				if (jansson) json_decref(jansson);
		...
		else (*workflow)->tasks[i].exit_action = 0; // Default value (no operation)

		if (!run_task) run_task = run;
		if (run_task && strlen(run_task))
		{
			if (!strcmp(run_task,OPH_WORKFLOW_NO)) (*workflow)->tasks[i].run = 0;
		...
		else (*workflow)->tasks[i].run = 1; // Default value (yes)
	...
	if (run && strlen(run))
	{
		if (!strcmp(run,OPH_WORKFLOW_NO)) (*workflow)->tasks[(*workflow)->tasks_num].run = 0;
		else if (strcmp(run,OPH_WORKFLOW_YES))
		{
			oph_workflow_free(*workflow);
			if (jansson) json_decref(jansson);
	...
	else (*workflow)->tasks[(*workflow)->tasks_num].run = 1; // Default value (yes)
	...
	//cwd
	if (!(*workflow)->cwd) (*workflow)->cwd = strdup(OPH_WORKFLOW_ROOT_FOLDER);
	...
	for (i = 0; i < (*workflow)->tasks_num; i++) (*workflow)->tasks[i].vars = hashtbl_create((*workflow)->tasks_num, NULL);
...
int oph_workflow_store(oph_workflow *workflow, char **jstring)
{
	...
	*jstring = NULL;
	...
	if (!request) return OPH_WORKFLOW_EXIT_MEMORY_ERROR;
	if (_oph_workflow_add_to_json(request, "name", workflow->name)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "author", workflow->author)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "abstract", workflow->abstract)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "sessionid", workflow->sessionid)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "exec_mode", workflow->exec_mode)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	snprintf(jsontmp,OPH_WORKFLOW_MIN_STRING,"%d",workflow->ncores);
	if (_oph_workflow_add_to_json(request, "ncores", jsontmp)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "on_error", workflow->on_error)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "on_exit", workflow->on_exit)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (!workflow->run && _oph_workflow_add_to_json(request, "run", "no")) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "cwd", workflow->cwd)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "cube", workflow->cube)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "callback_url", workflow->callback_url)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	if (_oph_workflow_add_to_json(request, "command", workflow->command)) return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	...
	for (i=0;i<workflow->tasks_num;++i)
	{
		...
		if (!task) break;
		if (_oph_workflow_add_to_json(task, "name", workflow->tasks[i].name)) break;
		if (_oph_workflow_add_to_json(task, "operator", workflow->tasks[i].operator)) break;
		if (_oph_workflow_add_to_json(task, "on_error", workflow->tasks[i].on_error)) break;
		if (_oph_workflow_add_to_json(task, "on_exit", workflow->tasks[i].on_exit)) break;
		if (!workflow->tasks[i].run && _oph_workflow_add_to_json(task, "run", "no")) break;

		json_t *arguments = json_array();
		if (!arguments) {
			if (task) json_decref(task);
			break;
		}
		for (j=0;j<workflow->tasks[i].arguments_num;++j)
		{
			snprintf(jsontmp,OPH_WORKFLOW_MAX_STRING,"%s=%s",workflow->tasks[i].arguments_keys[j],workflow->tasks[i].arguments_values[j]);
			if (json_array_append_new(arguments,json_string(jsontmp))) break;
		}
		...
		if (workflow->tasks[i].deps_num)
		{
			...
			for (j=0;j<workflow->tasks[i].deps_num;++j)
			{
				...
				if (!dependency) break;
				if (_oph_workflow_add_to_json(dependency, "argument", workflow->tasks[i].deps[j].argument)) break;
				if (_oph_workflow_add_to_json(dependency, "order", workflow->tasks[i].deps[j].order)) break;
				if (_oph_workflow_add_to_json(dependency, "task", workflow->tasks[i].deps[j].task_name)) break;
				if (_oph_workflow_add_to_json(dependency, "type", workflow->tasks[i].deps[j].type)) break;
				if (_oph_workflow_add_to_json(dependency, "filter", workflow->tasks[i].deps[j].filter)) break;
				if (_oph_workflow_add_to_json(dependency, "output_argument", workflow->tasks[i].deps[j].output_argument)) break;
				if (_oph_workflow_add_to_json(dependency, "output_order", workflow->tasks[i].deps[j].output_order)) break;

				if (json_array_append_new(dependencies,dependency))
				{
					if (dependency) json_decref(dependency);
					break;
				}
			}
			if (j<workflow->tasks[i].deps_num)
			{
				if (dependencies) json_decref(dependencies);
				if (task) json_decref(task);
				break;
			}
