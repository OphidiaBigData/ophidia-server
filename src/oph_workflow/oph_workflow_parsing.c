...
int oph_workflow_load(char *json_string, const char *username, const char *ip_address, oph_workflow ** workflow)
{
	...
	*workflow = NULL;
	...
	for (i = 0; i < (*workflow)->tasks_num; i++) {
		...
		char *name = NULL, *operator= NULL, *on_error_task = NULL, *on_exit_task = NULL, *run_task = NULL, *save_task = NULL;
		json_unpack(task, "{s?s,s?s,s?s,s?s,s?s,s?s}", "name", &name, "operator", &operator, "on_error", &on_error_task, "on_exit", &on_exit_task, "run", &run_task, "save", &save_task);
