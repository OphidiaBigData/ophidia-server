...
int oph_form_subm_string(const char *request, const int ncores, char *outfile, short int interactive_subm, oph_rmanager * orm, int jobid, const char *username, char **cmd, char type, int wid)
{
	if (!orm) {
		pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "Null input parameter\n");
		return RMANAGER_NULL_PARAM;
	...
	sprintf(*cmd, "%s %s %s %s%d %d %s \"%s\" %s %s%s %d %s", orm->subm_prefix, subm_username, command, internal_request ? "_" : "", jobid, ncores,
		outfile ? outfile : OPH_NULL_FILENAME, request, ncores == 1 ? orm->subm_queue_high : orm->subm_queue_low, oph_server_port, OPH_RMANAGER_PREFIX, wid, orm->subm_postfix);
...
int oph_serve_request(const char *request, const int ncores, const char *sessionid, const char *markerid, const char *error, struct oph_plugin_data *state, int *odb_wf_id, int *task_id,
		      int *light_task_id, int *odb_jobid, int delay, char **response, char **jobid_response, enum oph__oph_odb_job_status *exit_code, int *exit_output, char *username, int wid)
{
...
#else
	if (oph_form_subm_string(request, _ncores, outfile, 0, orm, odb_jobid ? *odb_jobid : 0, username, &cmd, 0, wid)) {
