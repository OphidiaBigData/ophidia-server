			...
			case 1:	// FIFO
				{
					...
					while ((oph_server_task_limit && (oph_server_task_running >= oph_server_task_limit))
						...
						if (just_arrived) {
							...
							just_arrived = 0;
							/*
							   pmesg(LOG_DEBUG, __FILE__, __LINE__, "Task %d enqueued%s\n", odb_jobid, priority >= 0 ? "" : " with high priority (%d-%d)", oph_server_task_running, oph_server_core_running);
			...
			case 2:	// LIFO
				{
					...
					oph_detached_task *tmp = NULL;
					while ((oph_server_task_limit && (oph_server_task_running >= oph_server_task_limit))
					       || (oph_server_core_limit && (oph_server_core_running + ncores > oph_server_core_limit)) || (oph_sched_queue_head && !just_arrived
																	    && (oph_sched_queue_head->id != odb_jobid))) {
						if (just_arrived) {
							tmp = (oph_detached_task *) malloc(sizeof(oph_detached_task));
							tmp->id = odb_jobid;
				...
				break;
			case 3:	// BIBO, considering the less value of priority the more priority
				{
					char just_arrived = 1;
					oph_detached_task *tmp = NULL, *tmp2 = NULL, *prev = NULL;
					while ((oph_server_task_limit && (oph_server_task_running >= oph_server_task_limit))
					       || (oph_server_core_limit && (oph_server_core_running + ncores > oph_server_core_limit)) || (oph_sched_queue_head
																	    && (just_arrived
																		&& (oph_sched_queue_head->priority <= priority)
																		|| (!just_arrived
																		    && (oph_sched_queue_head->id != odb_jobid))))) {
						if (just_arrived) {
							tmp = (oph_detached_task *) malloc(sizeof(oph_detached_task));
							...
							for (tmp2 = oph_sched_queue_head, prev = NULL; tmp2; prev = tmp2, tmp2 = tmp2->next)
								if (tmp2->priority > priority)
									break;
							tmp->next = tmp2;
							if (prev)
								prev->next = tmp;
								...
								oph_sched_queue_head = tmp;
							just_arrived = 0;
							/* pmesg(LOG_DEBUG, __FILE__, __LINE__, "Task %d enqueued (%d-%d)\n", odb_jobid, oph_server_task_running, oph_server_core_running);
							   if (get_debug_level() == LOG_DEBUG)
							   for (tmp = oph_sched_queue_head; tmp; tmp = tmp->next)
							   pmesg(LOG_DEBUG, __FILE__, __LINE__, "\t%d (%d)\n", tmp->id, tmp->priority); */
						}
						//pmesg(LOG_DEBUG, __FILE__, __LINE__, "Waiting for a task to be completed\n");
						pthread_cond_wait(&limit_flag, &global_flag);
						//pmesg(LOG_DEBUG, __FILE__, __LINE__, "A task has been completed\n");
					...
					if (oph_sched_queue_head && (oph_sched_queue_head->id == odb_jobid)) {
						...
						oph_sched_queue_head = oph_sched_queue_head->next;
						free(tmp);
						/* pmesg(LOG_DEBUG, __FILE__, __LINE__, "Task %d dequeued (%d-%d)\n", odb_jobid, oph_server_task_running, oph_server_core_running);
						   if (get_debug_level() == LOG_DEBUG)
						   for (tmp = oph_sched_queue_head; tmp; tmp = tmp->next)
						   pmesg(LOG_DEBUG, __FILE__, __LINE__, "\t%d (%d)\n", tmp->id, tmp->priority); /*
						   }
						...
						if (infile == NULL) {
							pmesg_safe(flag, LOG_WARNING, __FILE__, __LINE__, "Unable to open output file: %s\n", filename);
					...
					int oph_serve_request(const char *request, const int ncores, const char *sessionid, const char *markerid, const char *error, struct oph_plugin_data *state,
							      int *odb_wf_id, int *task_id, int *light_task_id, int *odb_jobid, int delay, char **response, char **jobid_response,
							      enum oph__oph_odb_job_status *exit_code, int *exit_output, char *username, char *project, char *taskname, int wid, char serial,
							      short priority) {
						pmesg_safe(&global_flag, LOG_DEBUG, __FILE__, __LINE__, "Incoming request '%s' to run job '%s#%s' with %d cores\n", request, sessionid, markerid,
							   ncores);
						...
						int result;
						if ((result =
						     oph_serve_known_operator(state, request, _ncores, sessionid, markerid, odb_wf_id, task_id, light_task_id, odb_jobid, response, jobid_response,
									      exit_code, exit_output, username, project, taskname)) != OPH_SERVER_UNKNOWN)
							return result;
...
#ifdef USE_MPI
						snprintf(command, OPH_MAX_STRING_SIZE, "rm -f %s; mpirun -np %d %s \"%s\" >> %s 2>> %s", outfile, _ncores, oph_operator_client, request, outfile,
							 outfile);
#else
