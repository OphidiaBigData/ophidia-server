	...
	*ext_path = OPH_MF_ROOT_FOLDER[0];
	...
	if (path && strlen(path))
	{
		...
		if (!first_nospace || (*first_nospace != OPH_MF_ROOT_FOLDER[0]))
		{
			...
			if (first_nospace && (*first_nospace != OPH_MF_ROOT_FOLDER[0])) strncat(ext_path,OPH_MF_ROOT_FOLDER,OPH_MAX_STRING_SIZE-strlen(ext_path));
		}
		strncat(ext_path,path,OPH_MAX_STRING_SIZE-strlen(ext_path));
	}
	else strncat(ext_path,cwd,OPH_MAX_STRING_SIZE-strlen(ext_path));
