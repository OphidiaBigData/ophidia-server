...
#define OPH_JSON_LOG_WRITE_ERROR	"JSON output cannot be written\n"
...
/**
 * \brief Function to create an OPH_JSON object from a JSON string.
 * \param json Pointer to the output OPH_JSON object (in case of error MUST be explicitly freed by the caller)
 * \param jstring Input JSON string (freed and NULLed before return)
 * \return 0 if successfull, N otherwise
 */
