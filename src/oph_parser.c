...
{
	if (!args) return OPH_SERVER_NULL_POINTER;
	*args=NULL;
...
{
	if (!list) return OPH_SERVER_NULL_POINTER;
	if (!(*list) || !((*list)->next)) return OPH_SERVER_OK;
	...
	do
	{
		...
		for (item=*list; item->next; )
		{
			...
			{
				count++;
				if (prev) prev->next = item->next; else *list = item->next;
...
{
	if (!task_string || !counter || !args) return OPH_SERVER_NULL_POINTER;
	...
	*counter=0;
	...
	if (!(*counter)) return OPH_SERVER_ERROR;
...
{
	if (!task_string || !args || !counter) return OPH_SERVER_NULL_POINTER;
	...
	*counter=0;
	...
	if (ptr1)
	{
		...
		if (*ptr1 == OPH_SEPARATOR_SUBPARAM_OPEN)
		{
			...
			if (!ptr3) return OPH_SERVER_ERROR;
			ptr2 = strchr(ptr3,OPH_SEPARATOR_PARAM[0]);
		}
		else ptr2 = strchr(ptr1,OPH_SEPARATOR_PARAM[0]);
		if (ptr2) ptr2++;
	}
...
{
	if (!args || !key || !value) return OPH_SERVER_NULL_POINTER;
...
int oph_set_arg(oph_argument** args, const char* key, const char* value)
{
	...
	for (tmp=*args;tmp;tmp=tmp->next)
	{
		if (!strncasecmp(tmp->key,key,OPH_MAX_STRING_SIZE)) break;
		tmp2 = tmp;
	}
...
{
	if (!string) return OPH_SERVER_NULL_POINTER;
	string[0] = '\0';

	oph_argument *tmp;
	for (tmp=args; tmp; tmp=tmp->next)
	{
		if (!strncasecmp(OPH_ARG_NCORES,tmp->key,OPH_MAX_STRING_SIZE)) continue;
		if (!strncasecmp(OPH_ARG_NHOSTS,tmp->key,OPH_MAX_STRING_SIZE)) continue;
		if (!strncasecmp(OPH_ARG_MODE,tmp->key,OPH_MAX_STRING_SIZE)) continue;
		if (!strncasecmp(OPH_ARG_CUBE,tmp->key,OPH_MAX_STRING_SIZE) && !add_datacube_input) continue;
		if (!strncasecmp(OPH_ARG_SRC_PATH,tmp->key,OPH_MAX_STRING_SIZE)) continue;
		if (!strncasecmp(OPH_ARG_MEASURE,tmp->key,OPH_MAX_STRING_SIZE)) continue;
		if (!strncasecmp(OPH_ARG_CALLBACK_URL,tmp->key,OPH_MAX_STRING_SIZE)) continue;
