...
int b64decode(const char* query, char** new_query)
{
	...
	if (!result_code)
	{
		if (len+1<OPH_MAX_PROGRAM_SIZE) *(query_+len)=0;
		else
		{
...
int oph_execute(struct soap *soap, char* server, xsd__string query, int b64)
{
	...
	if (b64 && !response.error && response.response)
	{
		...
		if (base64encode(response.response, strlen(response.response), _result, OPH_MAX_PROGRAM_SIZE)) return 1;
		printf("Return code: 0\nJobID: %s\nResponse: %s\n",response.jobid?response.jobid:"",result);
	}
	else printf("Return code: %ld\nJobID: %s\nResponse: %s\n",response.error,response.error || !response.jobid?"":response.jobid,response.error || !response.response?"":response.response);
...
int main(int argc, char* argv[])
{
  ...
  if (key) return_code = oph_get_configuration(&soap, server, key);
  else if (job) return_code = oph_notify(&soap, server, job, status);
  else return_code = oph_execute(&soap, server, query, b64);
...
{ 
  if (!file || !line) pmesg(LOG_ERROR, __FILE__,__LINE__,"");
...
void sigpipe_handle(int x) { pmesg(LOG_DEBUG, __FILE__, __LINE__, "CALLED oph_signal_handler; catched signal nr %d\n", x); }
