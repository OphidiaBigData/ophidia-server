...
int oph_odb_retrieve_user_id(ophidiadb *oDB, char* username, int *id_user) { return _oph_odb_retrieve_user_id(oDB, username, id_user, &global_flag); }
int oph_odb_retrieve_user_id_unsafe(ophidiadb *oDB, char* username, int *id_user) { return _oph_odb_retrieve_user_id(oDB, username, id_user, NULL); }
...
int _oph_odb_retrieve_session_id(ophidiadb *oDB, const char* sessionid, int *id_session, pthread_mutex_t* flag)
{
	...
	*id_session = 0;
	...
	if ((row = mysql_fetch_row(res)) != NULL) *id_session=(int)strtol(row[0], NULL, 10);
...
int oph_odb_retrieve_session_id(ophidiadb *oDB, const char* sessionid, int *id_session) { return _oph_odb_retrieve_session_id(oDB, sessionid, id_session, &global_flag); }
int oph_odb_retrieve_session_id_unsafe(ophidiadb *oDB, const char* sessionid, int *id_session) { return _oph_odb_retrieve_session_id(oDB, sessionid, id_session, NULL); }
...
int _oph_odb_retrieve_job_id(ophidiadb *oDB, char* sessionid, char* markerid, int* id_job, pthread_mutex_t* flag)
{
	...
	*id_job = 0;
	...
	if ((row = mysql_fetch_row(res)) != NULL) *id_job=(int)strtol(row[0], NULL, 10);

	mysql_free_result(res);

	return OPH_ODB_SUCCESS;
}
int oph_odb_retrieve_job_id(ophidiadb *oDB, char* sessionid, char* markerid, int* id_job) { return _oph_odb_retrieve_job_id(oDB, sessionid, markerid, id_job, &global_flag); }
int oph_odb_retrieve_job_id_unsafe(ophidiadb *oDB, char* sessionid, char* markerid, int* id_job) { return _oph_odb_retrieve_job_id(oDB, sessionid, markerid, id_job, NULL); }
...
int oph_odb_update_folder_table(ophidiadb* oDB, char* folder_name, int* id_folder) { return _oph_odb_update_folder_table(oDB, folder_name, id_folder, &global_flag); }
int oph_odb_update_folder_table_unsafe(ophidiadb* oDB, char* folder_name, int* id_folder) { return _oph_odb_update_folder_table(oDB, folder_name, id_folder, NULL); }
...
int oph_odb_update_session_table(ophidiadb* oDB, char* sessionid, int id_user, int* id_session) { return _oph_odb_update_session_table(oDB, sessionid, id_user, id_session, &global_flag); }
int oph_odb_update_session_table_unsafe(ophidiadb* oDB, char* sessionid, int id_user, int* id_session) { return _oph_odb_update_session_table(oDB, sessionid, id_user, id_session, NULL); }

int _oph_odb_update_job_table(ophidiadb* oDB, char* markerid, char* task_string, char* status, int id_user, int id_session, int nchildren, int* id_job, char *parentid, char* workflowid, pthread_mutex_t* flag)
{
	...
	{
		if (!task_string[i]) break;
		if (task_string[i] == '\'') new_query[j++] = '\\';
		new_query[j] = task_string[i];
		j++;
		if(j >= OPERATION_QUERY_SIZE - 1) break;
	}			
	new_query[j] = 0;

	if (parentid) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_CHILD, id_user, id_session, markerid, status, new_query, parentid, workflowid);
	else if (nchildren>=0) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_PARENT, id_user, id_session, markerid, status, new_query, nchildren, workflowid);
	else n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB, id_user, id_session, markerid, status, new_query);
...
int oph_odb_update_job_table(ophidiadb* oDB, char* markerid, char* task_string, char* status, int id_user, int id_session, int nchildren, int* id_job, char *parentid, char* workflowid) { return _oph_odb_update_job_table(oDB, markerid, task_string, status, id_user, id_session, nchildren, id_job, parentid, workflowid, &global_flag); }
int oph_odb_update_job_table_unsafe(ophidiadb* oDB, char* markerid, char* task_string, char* status, int id_user, int id_session, int nchildren, int* id_job, char *parentid, char* workflowid) { return _oph_odb_update_job_table(oDB, markerid, task_string, status, id_user, id_session, nchildren, id_job, parentid, workflowid, NULL); }
...
int _oph_odb_create_job(ophidiadb *oDB, char* task_string, HASHTBL *task_tbl, int nchildren, int* id_job, pthread_mutex_t* flag)
{
	...
	if ((res = _oph_odb_retrieve_job_id(oDB, sessionid, markerid, id_job, flag)))
	{
		...
		if ((res = _oph_odb_update_job_table(oDB, markerid, task_string, OPH_ODB_STATUS_PENDING_STR, id_user, id_session, nchildren, id_job, hashtbl_get(task_tbl, OPH_ARG_PARENTID), hashtbl_get(task_tbl, OPH_ARG_WORKFLOWID), flag)))
		{
...
int oph_odb_create_job(ophidiadb *oDB, char* task_string, HASHTBL *task_tbl, int nchildren, int* id_job) { return _oph_odb_create_job(oDB, task_string, task_tbl, nchildren, id_job, &global_flag); }
int oph_odb_create_job_unsafe(ophidiadb *oDB, char* task_string, HASHTBL *task_tbl, int nchildren, int* id_job) { return _oph_odb_create_job(oDB, task_string, task_tbl, nchildren, id_job, NULL); }

const char* oph_odb_convert_status_to_str(enum oph__oph_odb_job_status status)
{
	switch(status)
	{
		case OPH_ODB_STATUS_PENDING: return OPH_ODB_STATUS_PENDING_STR;
		case OPH_ODB_STATUS_RUNNING: return OPH_ODB_STATUS_RUNNING_STR;
		case OPH_ODB_STATUS_START: return OPH_ODB_STATUS_START_STR;
		case OPH_ODB_STATUS_SET_ENV: return OPH_ODB_STATUS_SET_ENV_STR;
		case OPH_ODB_STATUS_INIT: return OPH_ODB_STATUS_INIT_STR;
		case OPH_ODB_STATUS_DISTRIBUTE: return OPH_ODB_STATUS_DISTRIBUTE_STR;
		case OPH_ODB_STATUS_EXECUTE: return OPH_ODB_STATUS_EXECUTE_STR;
		case OPH_ODB_STATUS_REDUCE: return OPH_ODB_STATUS_REDUCE_STR;
		case OPH_ODB_STATUS_DESTROY: return OPH_ODB_STATUS_DESTROY_STR;
		case OPH_ODB_STATUS_UNSET_ENV: return OPH_ODB_STATUS_UNSET_ENV_STR;
		case OPH_ODB_STATUS_COMPLETED: return OPH_ODB_STATUS_COMPLETED_STR;
		case OPH_ODB_STATUS_ERROR: return OPH_ODB_STATUS_ERROR_STR;
		case OPH_ODB_STATUS_PENDING_ERROR: return OPH_ODB_STATUS_PENDING_ERROR_STR;
		case OPH_ODB_STATUS_RUNNING_ERROR: return OPH_ODB_STATUS_RUNNING_ERROR_STR;
		case OPH_ODB_STATUS_START_ERROR: return OPH_ODB_STATUS_START_ERROR_STR;
		case OPH_ODB_STATUS_SET_ENV_ERROR: return OPH_ODB_STATUS_SET_ENV_ERROR_STR;
		case OPH_ODB_STATUS_INIT_ERROR: return OPH_ODB_STATUS_INIT_ERROR_STR;
		case OPH_ODB_STATUS_DISTRIBUTE_ERROR: return OPH_ODB_STATUS_DISTRIBUTE_ERROR_STR;
		case OPH_ODB_STATUS_EXECUTE_ERROR: return OPH_ODB_STATUS_EXECUTE_ERROR_STR;
		case OPH_ODB_STATUS_REDUCE_ERROR: return OPH_ODB_STATUS_REDUCE_ERROR_STR;
		case OPH_ODB_STATUS_DESTROY_ERROR: return OPH_ODB_STATUS_DESTROY_ERROR_STR;
		case OPH_ODB_STATUS_UNSET_ENV_ERROR: return OPH_ODB_STATUS_UNSET_ENV_ERROR_STR;
		case OPH_ODB_STATUS_ABORTED: return OPH_ODB_STATUS_ABORTED_STR;
		case OPH_ODB_STATUS_SKIPPED: return OPH_ODB_STATUS_SKIPPED_STR;
		case OPH_ODB_STATUS_EXPIRED: return OPH_ODB_STATUS_EXPIRED_STR;
		case OPH_ODB_STATUS_CLOSED: return OPH_ODB_STATUS_CLOSED_STR;
		default: return OPH_ODB_STATUS_UNKNOWN_STR;
	}
...
int oph_odb_set_job_status_and_nchildrencompleted(int id_job, enum oph__oph_odb_job_status status, int nchildren, int force_nchildren_saving, ophidiadb *oDB)
{
	...
	switch (status)
	{
		...
		case OPH_ODB_STATUS_UNSET_ENV:
			if (nchildren<0) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_1, oph_odb_convert_status_to_str(status), id_job);
			else n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_PARENT_1, oph_odb_convert_status_to_str(status), nchildren, id_job);
			break;
		case OPH_ODB_STATUS_RUNNING:
			if (nchildren<0) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_2, oph_odb_convert_status_to_str(status), id_job);
			else if (force_nchildren_saving) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_PARENT_1, oph_odb_convert_status_to_str(status), nchildren, id_job);
			else n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_PARENT_2, oph_odb_convert_status_to_str(status), nchildren, id_job);
			break;
		...
		case OPH_ODB_STATUS_START_ERROR: // Exception due to errors in operator arguments
			if (nchildren<0) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_3, oph_odb_convert_status_to_str(status), id_job);
			else n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_PARENT_3, oph_odb_convert_status_to_str(status), nchildren, id_job);
			break;
		default:
			if (nchildren<0) n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_4, id_job);
			else n = snprintf(insertQuery, MYSQL_BUFLEN, MYSQL_QUERY_UPDATE_OPHIDIADB_JOB_STATUS_PARENT_4, nchildren, id_job);
	}
	if(n >= MYSQL_BUFLEN){
...
int oph_odb_update_session_label(ophidiadb* oDB, const char* sessionid, char* label) { return _oph_odb_update_session_label(oDB, sessionid, label, &global_flag); }
int oph_odb_update_session_label_unsafe(ophidiadb* oDB, const char* sessionid, char* label) { return _oph_odb_update_session_label(oDB, sessionid, label, NULL); }
