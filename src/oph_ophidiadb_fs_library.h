...
#define MYSQL_QUERY_OPH_FS_IS_VISIBLE_CONTAINER "SELECT idcontainer FROM container WHERE idfolder=%d AND containername='%s' AND hidden=0"
#define MYSQL_QUERY_OPH_FS_IS_HIDDEN_CONTAINER "SELECT idcontainer FROM container WHERE idfolder=%d AND containername='%s' AND hidden=1"
#define MYSQL_QUERY_OPH_FS_UNIQUENESS "SELECT folder.foldername FROM folder WHERE folder.idparent=%d AND folder.foldername='%s' UNION SELECT container.containername FROM container WHERE container.idfolder=%d AND container.hidden=0 AND container.containername='%s'"
#define MYSQL_QUERY_OPH_FS_UNIQUENESS_HIDDEN "SELECT container.containername FROM container WHERE container.idfolder=%d AND container.hidden=1 AND container.containername='%s'"
