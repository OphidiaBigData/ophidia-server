...
int oph_odb_fs_path_parsing(char *inpath,char *cwd,int *folder_id,char **output_path, ophidiadb *oDB) {
    ...
    while ((ptr2=strtok_r(NULL,"/",&savepointer))!=NULL) list_size++;
...
int oph_odb_fs_retrive_container_folder_id(ophidiadb *oDB, int container_id, int non_hidden, int *folder_id){
	...
	char query[MYSQL_BUFLEN];
	int n;
	if(non_hidden)	n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_RETRIEVE_CONTAINER_FOLDER_ID, container_id);
	else n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_RETRIEVE_CONTAINER_FOLDER_ID2, container_id);
	if(n >= MYSQL_BUFLEN){
		return OPH_ODB_STR_BUFF_OVERFLOW;
	}
...
int oph_odb_fs_str_last_token(const char *input, char **first_part, char **last_token) {
    ...
    *last_token = (char *)malloc(MYSQL_BUFLEN);
    ...
    while ((ptr2=strtok_r(NULL,"/",&savepointer))!=NULL) token_num++;
...
int oph_odb_fs_find_fs_objects(ophidiadb *oDB, int level, int id_folder, int hidden, char *container_name, MYSQL_RES **information_list)
{
	...
	if(level < 1) container_name = NULL;

	if(container_name) snprintf(where_clause, MYSQL_BUFLEN, "AND containername = '%s'", container_name);
	else where_clause[0] = 0;
	...
	switch(level){
		...
		case 1: 
			if(!hidden){
				if(container_name) n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_1_WC, id_folder, container_name);
				else n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_1, id_folder, id_folder);
			} 
			else{
				if(container_name) n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_1_H_WC, id_folder, container_name, id_folder, container_name);
				else n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_1_H, id_folder, id_folder, id_folder);
			} 
			break;
		case 2: 
			if(!hidden){
				if(container_name) n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2_WC, id_folder, container_name);
				else n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2, id_folder, id_folder);
			} 
			else{
				if(container_name) n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2_H_WC, id_folder, container_name, id_folder, container_name);
				else n = snprintf(query, MYSQL_BUFLEN, MYSQL_QUERY_FS_LIST_2_H, id_folder, id_folder, id_folder);			
			} 
			break;
		default: 
			return OPH_ODB_NULL_PARAM;
...
int oph_odb_fs_get_subfolders(int folder_id, int** subfolder_id, int* num_subfolders, ophidiadb *oDB)
{
	...
	*subfolder_id = (int*)malloc((*num_subfolders)*sizeof(int));
	...
	n=0;
	while ((row = mysql_fetch_row(res)) != NULL) (*subfolder_id)[n++] = (int)strtol(row[0], NULL, 10);
