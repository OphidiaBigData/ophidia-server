...
#define OPHIDIADB_RETRIEVE_TOTAL_RESERVED_PARTITIONS "SELECT COUNT(*) AS hosts, partitionname, username, STRCMP(MIN(status),'up') AS partitionstatus, partitiontype FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition INNER JOIN user ON hostpartition.iduser = user.iduser WHERE reserved AND username LIKE '%%%s%%' AND partitionname LIKE '%%%s%%' GROUP BY hostpartition.idhostpartition ORDER BY username, partitionname;"
