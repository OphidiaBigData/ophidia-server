...
#define MYSQL_RETRIEVE_PROGRESS_RATIO_OF_WORKFLOW "SELECT job.idjob, job.creationdate, NULL, hostxdatacube*dbmsxhost*dbxdbms*fragmentxdb AS fragment, COUNT(*) AS current FROM session INNER JOIN job ON session.idsession = job.idsession INNER JOIN task ON job.idjob = task.idjob INNER JOIN datacube ON idoutputcube = datacube.iddatacube INNER JOIN fragment ON datacube.iddatacube = fragment.iddatacube WHERE sessionid='%s' AND workflowid=%d GROUP BY fragment.iddatacube UNION SELECT ajob.idjob, ajob.creationdate, NULL, hostxdatacube*dbmsxhost*dbxdbms*fragmentxdb AS fragment, COUNT(*) AS current FROM session INNER JOIN jobaccounting AS ajob ON session.idsession = ajob.idsession INNER JOIN task ON ajob.idjob = task.idjob INNER JOIN datacube ON idoutputcube = datacube.iddatacube INNER JOIN fragment ON datacube.iddatacube = fragment.iddatacube WHERE sessionid='%s' AND workflowid=%d GROUP BY fragment.iddatacube;"
...
#define OPHIDIADB_RETRIEVE_RESERVED_HOSTS "SELECT COUNT(*) FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition WHERE status = 'up' AND reserved AND iduser = %d;"
#define OPHIDIADB_RETRIEVE_TOTAL_RESERVED_HOSTS "SELECT COUNT(*) FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition WHERE status = 'up' AND reserved;"
#define OPHIDIADB_RETRIEVE_RESERVED_PARTITIONS "SELECT COUNT(*), partitionname FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition WHERE status = 'up' AND reserved AND iduser = %d GROUP BY hostpartition.idhostpartition ORDER BY partitionname;"
#define OPHIDIADB_RETRIEVE_USERS "SELECT size, NULL, username, maxhosts FROM user LEFT JOIN (SELECT iduser, COUNT(*) AS size FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition WHERE status = 'up' AND reserved GROUP BY iduser) AS hosts ON user.iduser = hosts.iduser ORDER BY username;"
#define OPHIDIADB_RETRIEVE_TOTAL_RESERVED_PARTITIONS "SELECT COUNT(*) AS hosts, partitionname, username FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition INNER JOIN user ON hostpartition.iduser = user.iduser WHERE status = 'up' AND reserved GROUP BY hostpartition.idhostpartition ORDER BY username, partitionname;"
