...
#define MYSQL_RETRIEVE_WORKFLOWS_OF_SESSION "SELECT markerid, job.creationdate, status, workflowid FROM job INNER JOIN session ON job.idsession = session.idsession WHERE sessionid = '%s' AND idparent IS NULL ORDER BY markerid"
