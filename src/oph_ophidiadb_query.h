...
#define OPHIDIADB_RETRIEVE_RESERVED_PARTITIONS "SELECT COUNT(*), partitionname FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition WHERE status = 'up' AND reserved AND iduser = %d GROUP BY hostpartition.idhostpartition;"
#define OPHIDIADB_RETRIEVE_TOTAL_RESERVED_PARTITIONS "SELECT COUNT(*), partitionname, username, maxhosts FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hashost.idhostpartition = hostpartition.idhostpartition INNER JOIN user ON hostpartition.iduser = user.iduser WHERE status = 'up' AND reserved GROUP BY hostpartition.idhostpartition;"
