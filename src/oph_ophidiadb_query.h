...
#else
#define MYSQL_RETRIEVE_PROGRESS_RATIO_OF_WORKFLOW "SELECT job.idjob, job.creationdate, NULL FROM session INNER JOIN job ON session.idsession = job.idsession WHERE sessionid='%s' AND workflowid=%d  AND idparent IS NOT NULL UNION SELECT ajob.idjob, ajob.creationdate, NULL FROM session INNER JOIN jobaccounting AS ajob ON session.idsession = ajob.idsession WHERE sessionid='%s' AND workflowid=%d AND idparent IS NOT NULL;"
#endif
...
#define OPHIDIADB_RETRIEVE_USER "SELECT maxhosts, NULL, NULL FROM user WHERE iduser = %d;"
