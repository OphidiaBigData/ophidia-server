...
#define OPHIDIADB_FILL_PARTITION "INSERT INTO hashost (idhostpartition, idhost, importcount) SELECT LAST_INSERT_ID(), idhost, importcount FROM host WHERE idhost IN ( SELECT host.idhost FROM host INNER JOIN hashost ON host.idhost = hashost.idhost INNER JOIN hostpartition ON hostpartition.idhostpartition = hashost.idhostpartition WHERE partitionname = '%s' );"
