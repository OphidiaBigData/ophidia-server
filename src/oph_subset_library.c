...
{
	if (subset->type) { free(subset->type); subset->type=0; }
	if (subset->start) { free(subset->start); subset->start=0; }
	if (subset->end) { free(subset->end); subset->end=0; }
	if (subset->stride) { free(subset->stride); subset->stride=0; }
	if (subset->count) { free(subset->count); subset->count=0; }
}
...
int oph_subset_parse(const char* cond, unsigned long len, oph_subset* subset, long max)
{
	...
	if (!subset->number) return OPH_SUBSET_LIB_DATA_ERR;
	...
	while (next && (retval==OPH_SUBSET_LIB_OK))
	{
		...
		while (result2 && (retval==OPH_SUBSET_LIB_OK))
		{
			switch(number)
			{
				case 0:
					...
					else subset->end[i] = strtol(result2,NULL,10);
				...
				case 2:
					...
					if (subset->stride[i]>1) subset->type[i] = OPH_SUBSET_LIB_STRIDE;
					break;
				default:
		...
		if (retval!=OPH_SUBSET_LIB_OK) break;
		...
		++i;
		if (next) result = strchr(next,OPH_SUBSET_LIB_SUBSET_SEPARATOR[0]);
	}

	if (retval != OPH_SUBSET_LIB_OK) _oph_subset_free(subset);
