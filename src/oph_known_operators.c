...
int oph_finalize_known_operator(int idjob, oph_json *oper_json, const char *operator_name, char *error_message, int success, char **response, ophidiadb *oDB)
{
	char *jstring = NULL;
	if (oper_json)
	{
...
int oph_serve_known_operator(struct oph_plugin_data *state, const char* request, const int ncores, const char* sessionid, const char* markerid, int *odb_wf_id, int *task_id, int *light_task_id, int* odb_jobid, char** response, char** jobid_response)
{
	...
	if (!strncasecmp(operator_name,OPH_OPERATOR_CANCEL,OPH_MAX_STRING_SIZE))
	{
		...
		if (oph_finalize_known_operator(idjob, oper_json, operator_name, error_message, success, response, &oDB)) return OPH_SERVER_SYSTEM_ERROR;
	...
	else if (!strncasecmp(operator_name,OPH_OPERATOR_FOR,OPH_MAX_STRING_SIZE) || !strncasecmp(operator_name,OPH_OPERATOR_SET,OPH_MAX_STRING_SIZE))
	{
		...
		while (!success)
		{
			...
			if (!name)
			{
				pmesg(LOG_DEBUG, __FILE__, __LINE__, "Bad argument '%s'.\n",OPH_OPERATOR_PARAMETER_NAME);
				snprintf(error_message,OPH_MAX_STRING_SIZE,"Bad argument '%s'.",OPH_OPERATOR_PARAMETER_NAME);
				break;
			}
			if (j<wf->tasks[i].arguments_num)
			{
				pmesg(LOG_DEBUG, __FILE__, __LINE__, "Bad argument '%s'.\n",wf->tasks[i].arguments_keys[j]);
				snprintf(error_message,OPH_MAX_STRING_SIZE,"Bad argument '%s'.",wf->tasks[i].arguments_keys[j]);
				break;
			}
			if (mode)
			{
			...
			if (!mode && (error == OPH_SERVER_UNKNOWN) && (svalues_num > 0))
			{
				...
				{
					pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to push for-data into the stack of workflow '%s'.\n",wf->name);
	...
	else if (!strncasecmp(operator_name,OPH_OPERATOR_ENDFOR,OPH_MAX_STRING_SIZE))
	{
		...
		if (tmp)
		{
			...
			{
				pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to remove variable '%s' from environment of workflow '%s'.\n",tmp->name,wf->name);
				pthread_mutex_unlock(&global_flag);
				return OPH_SERVER_SYSTEM_ERROR;
			}
			if (tmp->index < tmp->values_num)
			{
				...
				{
					pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to update variable '%s' in environment of workflow '%s'.\n",tmp->name,wf->name);
					pthread_mutex_unlock(&global_flag);
					return OPH_SERVER_SYSTEM_ERROR;
				}
				...
				if (odb_jobid) // Reset status
				{
					...
					*task_id = p; // Used to change 'taskindex' in notification message to oph_for
					...
					if (oph_workflow_reset_task(wf, wf->tasks[p].dependents_indexes, wf->tasks[p].dependents_indexes_num, i, tmp, &tasks_num))
						pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to reset task data from '%s'.\n",wf->tasks[p].name);
			...
			{
				pmesg(LOG_WARNING, __FILE__, __LINE__, "Unable to pop for-data from the stack of workflow '%s'.\n",wf->name);
				pthread_mutex_unlock(&global_flag);
				return OPH_SERVER_SYSTEM_ERROR;
			}
		}
		...
		if (success) *error_message = 0;
		if (oph_finalize_known_operator(wf->tasks[i].idjob, oper_json, operator_name, error_message, success, response, &oDB)) return OPH_SERVER_SYSTEM_ERROR;
