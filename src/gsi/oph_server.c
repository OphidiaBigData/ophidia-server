...
void set_global_values(const char* configuration_file)
{
	...
	if ((value=hashtbl_get(oph_server_params, OPH_SERVER_CONF_POLL_TIME))) oph_server_poll_time=(unsigned int)strtol(value,NULL,10);
...
void cleanup()
{
  ...
  if (logfile) { fclose(logfile); fclose(stdout); fclose(stderr); }
...
void oph_signal_handler(int signal)
{
	...
	if (errno != ECHILD) pmesg(LOG_ERROR, __FILE__,__LINE__,"An error occurred on shutdown\n");
	exit(0);
}
void oph_child_signal_handler (int sig) { UNUSED(sig) }
...
process_request(void *arg)
{
        ...
        if (!rc) /* success */
	{
		...
		{
			int i;
			for(i = 0; data->fqans[i]; i++) pmesg_safe(&global_flag, LOG_DEBUG, __FILE__,__LINE__,"FQAN[%d]: %s\n", i, data->fqans[i]);
		}
#endif
...
int oph_utils_tokenize_string_r(const char *s, const char *delimiters, char ***tokenp)
{
	...
	*tokenp = NULL;
	...
	if (!(t = (char*)calloc(strlen(snew) + 1, sizeof(char)))) numtokens = -1;
	else /* count the number of tokens in snew */
	{
		strcpy(t, snew);
		if ( !(ptr=strtok_r(t, delimiters, &saveptr))) numtokens = 0;
		else
		{
			for (numtokens = 1; (ptr=strtok_r(NULL, delimiters, &saveptr)) != NULL; numtokens++);
			if (!(*tokenp = (char**)calloc(numtokens + 1, sizeof(char *)))) numtokens = -1;
...
int load_voms_system_privileges (struct oph_plugin_data *data, char **fqans)
{
	...
	if ( ( (numtoken = oph_utils_tokenize_string_r(data->serverid, "/", &token)) < 2) || !token || !token[1])
	{
		...
		if (token)
		{
			for (j = 0; token[j]; j++) if (token[j]) free(token[j]);
			free (token);
		}
		return -2;
	}
	...
	if(fqans)
	{
		...
		for (i = 0; fqans[i] && (data->authorization < OPH_AUTH_WRITE); i++)
		{
			...
			for (j = 0; token[j]; j++) pmesg_safe(&global_flag,LOG_DEBUG, __FILE__,__LINE__,"Token %d: %s\n", j, token[j]);
			...
			{                
				...
				else if (!strncasecmp(token[j], "Capability=", 11)) continue;
				else if (j == 3) server_name = strdup(token[3]);
				else
				{
			...
			if (!fd)
			{
				...
				if (!fd) pmesg_safe(&global_flag,LOG_DEBUG,__FILE__,__LINE__,"Can not read file '%s'\n", auth_vo_file);
				if (auth_vo) free(auth_vo);
				if (auth_vo_file) free(auth_vo_file);
			}
			...
			if (!strcasecmp(role,OPH_VOMS_AUTH_READ) && data->authorization < OPH_AUTH_READ) data->authorization = OPH_AUTH_READ;
			else if (!strcasecmp(role,OPH_VOMS_AUTH_WRITE) && data->authorization < OPH_AUTH_WRITE) data->authorization = OPH_AUTH_WRITE;
	...
	else pmesg_safe(&global_flag,LOG_DEBUG, __FILE__,__LINE__,"No FQANS found!\n");
	if (hostname) free(hostname);
...
int main(int argc, char **argv)
{
	...
	else oph_log_file_name = hashtbl_get(oph_server_params, OPH_SERVER_CONF_LOGFILE);
...
int gsi_authorization_callback(struct soap *soap, char *distinguished_name)
{
	...
	if (!data->authorization)
	{
		...
		else pmesg_safe(&global_flag,LOG_DEBUG,__FILE__,__LINE__,"Can not read file '%s'\n", black_list_file);
		if (black_list) free(black_list);
		if (black_list_file) free(black_list_file);
	}
	...
	if (!data->authorization)
	{
		...
		if (!data->authorization) pmesg_safe(&global_flag,LOG_DEBUG,__FILE__,__LINE__,"Local authorization is denied\n");
	}
