...
int oph_workflow_execute(struct oph_plugin_data *state, char ttype, int jobid, oph_workflow * wf, int *tasks_indexes, int tasks_indexes_num, ophidiadb * oDB, char **jobid_response)
{
	...
	char *submission_string, *sss, *errore, str_markerid[OPH_SHORT_STRING_SIZE], str_workflowid[OPH_SHORT_STRING_SIZE], oph_jobid[OPH_MAX_STRING_SIZE], submission_string_ext[OPH_MAX_STRING_SIZE],
	    *output_json = NULL;
			...
			while (!success) {
				...
				while (wtmp) {
					if (wtmp->status && (wtmp->status < OPH_ODB_STATUS_ABORTED))	// Discard uninitialized or aborted jobs
					{
						...
						jjj = 0;
						snprintf(jsontmp, OPH_MAX_STRING_SIZE, "%s%s%d%s%d", wf->sessionid, OPH_SESSION_WORKFLOW_DELIMITER, wf->workflowid, OPH_SESSION_MARKER_DELIMITER,
							 wtmp->markerid);
						jsonvalues[jjj] = strdup(jsontmp);
						if (!jsonvalues[jjj]) {
							pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "%c%d: Error allocating memory\n", ttype, jobid);
