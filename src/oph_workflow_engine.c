		...
		if (datacube_inputs) {
			...
			if (running) {
				...
				for (i = 0; i < (int) number; ++i) {
					...
					task->light_tasks[i].proc_time = NULL;
					for (j = 0; j < task->arguments_num; ++j) {
						...
						if ((src_path && (task->arguments_keys[j] == src_path_key)) || (!src_path && (task->arguments_keys[j] == datacube_input_key))) {
							...
							if (task->light_tasks[i].input)
								free(task->light_tasks[i].input);
							task->light_tasks[i].input = strdup(datacube_inputs[i]);
...
int oph_workflow_parallel_fco(oph_workflow *wf, int nesting_level, struct oph_plugin_data *state)
{
	...
	if (!wf || !wf->tasks) {
		pmesg_safe(flag, LOG_ERROR, __FILE__, __LINE__, "Null param\n");
		return OPH_WORKFLOW_EXIT_BAD_PARAM_ERROR;
	}
	...
	for (i = 0; i < wf->tasks_num; ++i) {
		if (!strncasecmp(wf->tasks[i].operator, OPH_OPERATOR_FOR, OPH_WORKFLOW_MAX_STRING)) {
				...
				if (jj >= 0) {
					for (j = 0; j < wf->tasks[i].arguments_num; ++j)
						if (!strcasecmp(wf->tasks[i].arguments_keys[j], OPH_ARG_VALUES)) {
							pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Override value of parameter '%s' of task '%s' with '%s'\n", OPH_ARG_VALUES, wf->tasks[i].name,
								   wf->tasks[i].arguments_values[jj]);
							free(wf->tasks[i].arguments_values[j]);
							wf->tasks[i].arguments_values[j] = strdup(wf->tasks[i].arguments_values[jj]);
							break;
						}
					if (j >= wf->tasks[i].arguments_num) {
						pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Override key of parameter '%s' of task '%s' with '%s'\n", wf->tasks[i].arguments_keys[jj],
			...
			if (j < wf->tasks[i].arguments_num) {
				pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Bad argument '%s' of task '%s'.\n", OPH_OPERATOR_PARAMETER_PARALLEL, wf->tasks[i].name);
				break;
			}
			if (!wf->tasks[i].parallel_mode)
				continue;
			...
			if (j < wf->tasks[i].arguments_num) {
				pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Generic error in parsing arguments of task '%s'.\n", wf->tasks[i].name);
				break;
			}
			for (j = 0; j < wf->tasks[i].arguments_num; ++j) {
			...
			if (svalues_num) {
				if (ivalues_num && (ivalues_num != svalues_num)) {
					pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Arguments '%s' and '%s' have different sizes.\n", OPH_ARG_VALUES, OPH_OPERATOR_PARAMETER_COUNTER,
						   wf->tasks[i].name);
					break;
				}
			} else
			...
			if (!hashtbl_get(wf->vars, number_of_loops)) {
				...
				if (!var.svalue) {
					pmesg_safe(flag, LOG_ERROR, __FILE__, __LINE__, "Memory error\n");
					break;
				}
				...
				if (hashtbl_insert_with_size(wf->vars, number_of_loops, var_buffer, var_size + svalue_size)) {
					pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Unable to store variable '%s' in environment of workflow '%s'. Maybe it already exists.\n", number_of_loops,
						   wf->name);
					free(var.svalue);
					free(var_buffer);
					break;
				}
				pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Added variable '%s=%s' in environment of workflow '%s'.\n", number_of_loops, var.svalue, wf->name);
				free(var.svalue);
				free(var_buffer);
			}
				...
				for (j = 0; j < wf->tasks_num; ++j)
					if (wf->tasks[j].is_marked) {
						...
						if (hashtbl_insert_with_size(wf->tasks[j].vars, name, var_buffer, var_size + svalue_size)) {
							pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Unable to store variable '%s' in environment of task '%s'. Maybe it already exists.\n", name,
								   wf->tasks[j].name);
							free(var.svalue);
							free(var_buffer);
							break;
						}
						pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Added variable '%s=%s' in environment of task '%s'.\n", name, var.svalue, wf->tasks[j].name);
						free(var.svalue);
						free(var_buffer);
					}
				continue;
			...
			pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "%d task%s will be replied %d time%s.\n", replied_num, replied_num == 1 ? "" : "s", new_branch_num,
				   new_branch_num == 1 ? "" : "s");
				...
				for (kk = kkk = 0; kk < replied_num; kkk++) {
					if (wf->tasks[kkk].is_marked) {
						...
						if (oph_workflow_copy_task(wf->tasks + kkk, wf->tasks + kkkk, 2 + j)) {
							pmesg_safe(flag, LOG_WARNING, __FILE__, __LINE__, "Error in copy task '%s'.\n", wf->tasks[kkk].name);
							break;
						}
						pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Task '%s' (index %d) has been copied with index %d.\n", wf->tasks[kkk].name, kkk, kkkk);
						if (!j)
							new_index[kkk] = kk;
						kk++;
					}
			...
			for (j = 0; j < wf->tasks_num; ++j)
				if (wf->tasks[j].is_marked) {
					...
					if (svalues)
						...
						pmesg_safe(flag, LOG_DEBUG, __FILE__, __LINE__, "Added variable '%s=%d' in environment of task '%s'.\n", name, var.ivalue, wf->tasks[j].name);
					free(var.svalue);
					free(var_buffer);
				}
		...
		for (k = 0; k < tasks_indexes_num; ++k) {
			...
			if ((wf->tasks[i].parent < 0) && ((res = oph_check_for_massive_operation(state, ttype, jobid, wf, i, oDB, &output_list, &output_list_dim, NULL, &remake_submission_string)))) {
				...
				if (res == OPH_SERVER_NO_RESPONSE) {
					...
					if (!success)
						...
						char **jsonkeys = NULL, **fieldtypes = NULL, is_compact = 1, is_extended = 1;
						switch (wf->output_format) {
							case 3:	// CLASSIC
								num_fields = 6;
								is_compact = 0;
								is_extended = 0;
						...
						while (!success) {
							...
							if (is_extended) {
								...
								jjj++;
								jsonkeys[jjj] = strdup("BEGIN TIME");
								if (!jsonkeys[jjj]) {
									pmesg(LOG_ERROR, __FILE__, __LINE__, "%c%d: Error allocating memory\n", ttype, jobid);
									for (iii = 0; iii < jjj; iii++)
										if (jsonkeys[iii])
								...
								jjj++;
								jsonkeys[jjj] = strdup("PROCESS TIME");
								if (!jsonkeys[jjj]) {
									pmesg(LOG_ERROR, __FILE__, __LINE__, "%c%d: Error allocating memory\n", ttype, jobid);
									for (iii = 0; iii < jjj; iii++)
										if (jsonkeys[iii])
			...
			if (wf->tasks[i].light_tasks_num)	// Massive operation
			{
				...
				for (j = 0; j < wf->tasks[i].light_tasks_num; ++j) {
					...
					request_data[k][j].serial = wf->tasks[i].serial;
				...
				if (!retry) {
					...
					*buffer = 0;
					...
					if (localtime_r(&nowtime, &nowtm)) {
						...
						wf->tasks[i].begin_time = strdup(buffer);
					}
				} else
			...
			} else	// Single operation
			{
				...
				request_data[k]->serial = wf->tasks[i].serial;
			...
			if ((status == OPH_ODB_STATUS_COMPLETED) || (status == OPH_ODB_STATUS_ERROR)) {
				...
				if (status == OPH_ODB_STATUS_COMPLETED) {
					...
					if (outputs_keys) {
						...
						for (i = 0; i < outputs_num; ++i) {
							...
							} else if (!strncmp(outputs_keys[i], OPH_ARG_TIME, OPH_MAX_STRING_SIZE)) {
								wf->tasks[task_index].light_tasks[light_task_index].proc_time = strdup(outputs_values[i]);
				...
				if (!wf->tasks[task_index].residual_light_tasks_num)	// Massive operation is ended
				{
					...
					if (!success)
						...
						int num_fields, iii, jjj = 0;
						char **jsonkeys = NULL, **fieldtypes = NULL, **jsonvalues = NULL, jsontmp[OPH_MAX_STRING_SIZE], is_compact = 1, is_extended = 1;
						switch (wf->output_format) {
							case 3:	// CLASSIC
								num_fields = 6;
								is_compact = 0;
							...
							for (i = 0; i < wf->tasks[task_index].light_tasks_num; ++i) {
								...
								if (wf->tasks[task_index].light_tasks[i].status && wf->tasks[task_index].light_tasks[i].markerid
									...
									if (is_extended) {
										...
										jjj++;
										jsonvalues[jjj] =
										    strdup(wf->tasks[task_index].light_tasks[i].output ? wf->tasks[task_index].light_tasks[i].output : "");
										if (!jsonvalues[jjj]) {
											pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
											for (iii = 0; iii < jjj; iii++)
												if (jsonvalues[iii])
										...
										jjj++;
										jsonvalues[jjj] =
										    strdup(wf->tasks[task_index].light_tasks[i].proc_time ? wf->tasks[task_index].light_tasks[i].proc_time : "0.0");
										if (!jsonvalues[jjj]) {
											pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
											for (iii = 0; iii < jjj; iii++)
												if (jsonvalues[iii])
				...
				if (task_index == wf->tasks_num) {
					...
					for (i = 0; i < outputs_num; ++i) {
						if (!strncmp(outputs_keys[i], OPH_ARG_CUBE, OPH_MAX_STRING_SIZE)) {
							...
							wf->tasks[task_index].proc_time = strdup(outputs_values[i]);
						} else if (!strncmp(outputs_keys[i], OPH_ARG_INPUT, OPH_MAX_STRING_SIZE)) {
							for (j = 0; j < wf->tasks[task_index].arguments_num; ++j)
								if (!strncmp(wf->tasks[task_index].arguments_keys[j], OPH_ARG_INPUT, OPH_MAX_STRING_SIZE)
								    || !strncmp(wf->tasks[task_index].arguments_keys[j], OPH_ARG_SRC_PATH, OPH_MAX_STRING_SIZE)) {
			...
			while (!success) {
				...
				while (wtmp) {
					...
					if (wtmp->status && wtmp->markerid && ((wtmp->status < OPH_ODB_STATUS_ABORTED) || (wtmp->status == OPH_ODB_STATUS_UNSELECTED))) {
						...
						if (is_extended) {
							jjj++;
							jsonvalues[jjj] = strdup(wtmp->input);
							if (!jsonvalues[jjj]) {
								pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "%c%d: Error allocating memory\n", ttype, jobid);
								for (iii = 0; iii < jjj; iii++)
									if (jsonvalues[iii])
							...
							jjj++;
							jsonvalues[jjj] = strdup(wtmp->proc_time);
							if (!jsonvalues[jjj]) {
								pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "%c%d: Error allocating memory\n", ttype, jobid);
								for (iii = 0; iii < jjj; iii++)
									if (jsonvalues[iii])
