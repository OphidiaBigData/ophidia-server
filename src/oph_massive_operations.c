...
void freeBlock(char*** block, unsigned int count)
{
	...
	{
		unsigned int i;
		for (i=0;i<count;++i) if ((*block)[i]) free((*block)[i]);
		free(*block);
		*block = NULL;
	}
}
...
{
	if (!path || !counter || !buffer) return OPH_SERVER_NULL_POINTER;

	DIR* dirp = opendir(path);
	if (!dirp) return OPH_SERVER_WRONG_PARAMETER_ERROR;
...
int _oph_mf_parse_KV(char*** datacube_inputs, char*** measure_name, unsigned int* counter, char* task_string, char* cwd, char* sessionid, int* running, int is_src_path, ophidiadb* oDB, char** _query, pthread_mutex_t* flag)
{
	if (!task_string || !datacube_inputs || !measure_name || !counter || !sessionid || !running || !oDB) return OPH_SERVER_NULL_POINTER;
	...
	*running = 1;
	...
	if (!strchr(task_string,OPH_SEPARATOR_KV[0]))
	{
		...
		{
			unsigned int i,j;
			for (i=j=0;(i<OPH_MAX_STRING_SIZE) && (j<strlen(task_string));j++) if ((task_string[j]!=OPH_SEPARATOR_NULL) && (task_string[j]!=OPH_SEPARATOR_PARAM[0])) tmp[i++]=task_string[j];
			tmp[i]=0;
			switch (strlen(tmp))
			{
				case 0:
				...
				case 1:
					...
					break;
				case 3:
					if (!strcasecmp(tmp,"all")) filter=0;
					break;
				default:
	...
	if (!is_src_path)
	{
		...
		if (_query) *_query = strdup(query);
	}

	char* running_value = task_tbl ? hashtbl_get(task_tbl, OPH_MF_ARG_RUN) : NULL;
	if (running_value && !strncasecmp(running_value,OPH_MF_ARG_VALUE_NO,OPH_MAX_STRING_SIZE)) *running = 0;
		...
		if (strchr(path,'*') || strchr(path,'~') || strchr(path,'{') || strchr(path,'}')) // Use glob
		{
			...
			*counter = globbuf.gl_pathc;
			for (i=0; i<globbuf.gl_pathc; ++i)
			{
				...
				else s = asprintf(&tbuffer,"%s%s",real_path,OPH_SEPARATOR_PARAM);
			}
			globfree(&globbuf);
		}
		else
	...
	convention[0]=0;
	if ((running_value = task_tbl ? hashtbl_get(task_tbl, OPH_MF_ARG_CONVENTION) : NULL)) strncat(convention,running_value,OPH_MAX_STRING_SIZE-1);

	if (task_tbl) hashtbl_destroy(task_tbl);

	int lbuffer = 0;
	if (tbuffer) lbuffer = strlen(tbuffer);
	...
	*datacube_inputs = (char**)malloc((*counter)*sizeof(char*));
	...
	for (i=0; i<*counter; ++i)
	{
		...
		else snprintf(query,OPH_MAX_STRING_SIZE,"%s/%d/%d",oph_web_server,datacube[i].id_container,datacube[i].id_datacube);
		...
		if (is_src_path) // Check for file convention
		{
			if (!strncasecmp(convention,OPH_MF_ARG_VALUE_CMIP5,OPH_MAX_STRING_SIZE)) // CMIP5 convention
			{
				...
				if (base_name)
				{
					...
					if (measure) *measure=0; // Truncate file name to variable name only, which is the first word until the first occurrence of '_'
					(*measure_name)[i] = strdup(base_name);
				}
				else (*measure_name)[i]=0;
...
int _oph_mf_parse_query(char*** datacube_inputs, char*** measure_name, unsigned int* counter, char* datacube_input, char* cwd, char* sessionid, int* running, int is_src_path, ophidiadb* oDB, char** query, pthread_mutex_t* flag)
{
	if (!datacube_input || !datacube_inputs || !measure_name || !counter || !sessionid || !running || !oDB) return OPH_SERVER_NULL_POINTER;
	*datacube_inputs = *measure_name = NULL;
	*counter = 0;
	*running = -1;
	...
	{
		if (datacube_input[i]==OPH_SEPARATOR_BASIC[0]) result=0;
		if (datacube_input[i]==OPH_SEPARATOR_KV[0]) result=1;
		...
		*end_task=OPH_SEPARATOR_PARAM[0];
		...
		if (counter_>0)
		{
			if (*counter)
			{
				...
				tmp1 = *datacube_inputs; tmp2 = *measure_name;
				*datacube_inputs = (char**)malloc((*counter + counter_)*sizeof(char*));
				if (tmp2) *measure_name = (char**)malloc((*counter + counter_)*sizeof(char*));
				for (i=0;i<*counter;++i)
				{
					(*datacube_inputs)[i] = tmp1[i];
					if (tmp2) (*measure_name)[i] = tmp2[i];
				}
				for (j=0;j<counter_;++j,++i)
				{
					(*datacube_inputs)[i] = datacube_inputs_[j];
					if (tmp2 && measure_name_) (*measure_name)[i] = measure_name_[j];
				}

				free(tmp1);
				if (tmp2) free(tmp2);
				free(datacube_inputs_); datacube_inputs_ = NULL;
				if (measure_name_) { free(measure_name_); measure_name_ = NULL; }
			}
			else
			{
	...
	while (end_task)
	{
		if (*end_task == OPH_SEPARATOR_SUBPARAM) *end_task = OPH_SEPARATOR_NULL;
		if (*end_task == OPH_SEPARATOR_SUBPARAM_CLOSE) end_task = strchr(end_task, OPH_SEPARATOR_SUBPARAM_OPEN);
		else end_task++;
	}
	...
	if (!end_task || (task_string && (end_task > task_string))) counter_++;
	while (task_string)
	{
	...
	else *datacube_inputs = (char**)malloc(counter_*sizeof(char*));
	*counter += counter_;
...
int oph_mf_parse_query(char*** datacube_inputs, char*** measure_name, unsigned int* counter, char* datacube_input, char* cwd, char* sessionid, int* running, int is_src_path, ophidiadb* oDB, char** query)
{
...
int oph_mf_parse_query_unsafe(char*** datacube_inputs, char*** measure_name, unsigned int* counter, char* datacube_input, char* cwd, char* sessionid, int* running, int is_src_path, ophidiadb* oDB, char** query)
{
