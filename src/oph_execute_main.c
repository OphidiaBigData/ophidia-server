											...
											for (i = 0; i < item->wf->tasks[task_index].light_tasks_num; ++i) {
												if (item->wf->tasks[task_index].light_tasks[i].status && oph_check_status_mask(item->wf->tasks[task_index].light_tasks[i].status, smask))	// Discard uninitialized or aborted jobs
												{
													...
													if (is_extended) {
														...
														jjj++;
														jsonvalues[jjj] =
														    strdup(item->wf->tasks[task_index].light_tasks[i].output ? item->wf->
															   tasks[task_index].light_tasks[i].output : "");
														if (!jsonvalues[jjj]) {
															pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
															for (iii = 0; iii < jjj; iii++)
																if (jsonvalues[iii])
														...
														jjj++;
														jsonvalues[jjj] =
														    strdup(item->wf->tasks[task_index].begin_time ? item->wf->tasks[task_index].
															   begin_time : "");
														if (!jsonvalues[jjj]) {
															pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
															for (iii = 0; iii < jjj; iii++)
														...
														jjj++;
														jsonvalues[jjj] =
														    strdup(item->wf->tasks[task_index].light_tasks[i].end_time ? item->wf->
															   tasks[task_index].light_tasks[i].end_time : "");
														if (!jsonvalues[jjj]) {
															pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
															for (iii = 0; iii < jjj; iii++)
																if (jsonvalues[iii])
