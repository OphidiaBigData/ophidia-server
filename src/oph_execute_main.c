...
void free_string_vector(char** ctime, int n)
{
	...
	{
		int i;
		for (i=0;i<n;++i) if (ctime[i]) free(ctime[i]);
		free(ctime);
	}
}
...
int oph_check_operator(const char* operator, int* ncores, int* role)
{
	...
	*value=*op_role=0;
	...
	if (result == OPH_TP_TASK_SYSTEM_ERROR) return OPH_SERVER_WRONG_PARAMETER_ERROR;
	if (strlen(value)) *ncores = (int)strtol(value, NULL, 10);
	else *ncores = OPH_DEFAULT_CORES;
...
int oph__ophExecuteMain(struct soap *soap, xsd__string request, struct oph__ophResponse* response)
{
  ...
  if (!request || !response)
  {
	...
	if (response) response->error = OPH_SERVER_NULL_POINTER;
	return SOAP_OK;
  }
  ...
  pmesg_safe(&global_flag, LOG_INFO, __FILE__,__LINE__, "R%d: assigned label R%d to workflow:\n%s\n", jobid, jobid, request);
  ...
  j=strlen(_userid);
  for (i=0;i<j;++i) if ((_userid[i]=='/') || (_userid[i]==' ') || (_userid[i]=='=')) _userid[i]='_';
  ...
  // Load workflow
	...
	if (oph_workflow_load(request,soap->userid,&wf))
	{
		...
		if (json) free(json);
		if (wf->author) { free(wf->author); wf->author=NULL; }
#else
		pmesg_safe(&global_flag, LOG_WARNING, __FILE__,__LINE__,"R%d: received wrong data\n", jobid);
		response->error = OPH_SERVER_WRONG_PARAMETER_ERROR;
		return SOAP_OK;
	...
	if (wf->cwd) { free(wf->cwd); wf->cwd=NULL; }
	if (wf->cube) { free(wf->cube); wf->cube=NULL; }
  ...
  for (i=0; i<wf->tasks_num; ++i)
  {
	...
	if (ncores < wf->tasks[i].ncores) ncores = wf->tasks[i].ncores;
  }
  if (!ncores)
  {
	wf->ncores = ncores = OPH_DEFAULT_CORES;
	for (i=0; i<wf->tasks_num; ++i) wf->tasks[i].ncores = ncores;
  }
  else wf->ncores = ncores;
  for (i=0; i<wf->tasks_num; ++i) pmesg_safe(&global_flag, LOG_DEBUG, __FILE__,__LINE__, "R%d: %s=%d for task '%s'\n",jobid,OPH_ARG_NCORES,wf->tasks[i].ncores,wf->tasks[i].name);
  ...
  for (i=0; i<wf->tasks_num; ++i)
  {
	...
	else if (!strncasecmp(wf->tasks[i].operator,OPH_OPERATOR_GET_CONFIG,OPH_MAX_STRING_SIZE)) oph_known_operator = OPH_GET_CONFIG_OPERATOR;
	else if (!strncasecmp(wf->tasks[i].operator,OPH_OPERATOR_RESUME,OPH_MAX_STRING_SIZE)) oph_known_operator = OPH_RESUME_OPERATOR;
	else if (!strncasecmp(wf->tasks[i].operator,OPH_OPERATOR_MANAGE_SESSION,OPH_MAX_STRING_SIZE)) oph_known_operator = OPH_MANAGE_SESSION_OPERATOR;
	else if (!strncasecmp(wf->tasks[i].operator,OPH_OPERATOR_CANCEL,OPH_MAX_STRING_SIZE)) oph_known_operator = OPH_CANCEL_OPERATOR;
	else nstandardcommands++;
  }
  ...
  if (oph_known_operator == OPH_SERVICE_OPERATOR)
  {
	...
	for (i=0;i<wf->tasks[0].arguments_num;++i) if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i],OPH_ARG_STATUS,OPH_MAX_STRING_SIZE)) break;
	if (i<wf->tasks[0].arguments_num)
	{
		...
		{
			pthread_mutex_lock(&global_flag);
			if (!strncasecmp(value,OPH_OPERATOR_SERVICE_PARAMETER_STATUS_UP,OPH_MAX_STRING_SIZE)) oph_service_status=1;
			else if (!strncasecmp(value,OPH_OPERATOR_SERVICE_PARAMETER_STATUS_DOWN,OPH_MAX_STRING_SIZE)) oph_service_status=0;
			else
			{
	...
	*tmp=0;
	...
	if (oper_json)
	{
		...
		if (!success)
		{
			...
			else if (oph_json_to_json_string(oper_json,&jstring)) return_code = -1;
		}
		else
		{
		...
		if (!return_code) pmesg_safe(&global_flag, LOG_DEBUG, __FILE__,__LINE__, "R%d: JSON output created\n", jobid);
		if (jstring)
		{
  ...
  if (oph_known_operator == OPH_GET_CONFIG_OPERATOR)
  {
	...
	*tmp=0;
	...
	pmesg_safe(&global_flag, LOG_DEBUG, __FILE__,__LINE__, "R%d: check for %s\n",jobid,OPH_ARG_KEY);
	for (i=0;i<wf->tasks[0].arguments_num;++i) if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i],OPH_ARG_KEY,OPH_MAX_STRING_SIZE)) break;
	if (i<wf->tasks[0].arguments_num) key = wf->tasks[0].arguments_values[i];
		...
		while(!success)
		{
			...
			for (i=0;i<OPH_USER_LAST_STATUS_SIZE; ++i)
			{
				...
				if (!oph_get_arg(user_args,parameters[i],tmp))
				{
					...
					if (!last_session_id && !strcmp(parameters[i],OPH_USER_LAST_SESSION_ID) && (strlen(tmp)>0)) last_session_id = strdup(tmp);
				}
			}
			if (i<OPH_USER_LAST_STATUS_SIZE)
			{
				if (last_session_id) free(last_session_id);
				break;
			}
			...
			if (last_session_id)
			{
				...
				for (i=0;i<OPH_SESSION_LAST_STATUS_SIZE; ++i)
				{
					...
					{
						if (!key || !strncasecmp(key,OPH_OPERATOR_GET_CONFIG_PARAMETER_ALL,OPH_MAX_STRING_SIZE) || !strncasecmp(key,sparameters[i],OPH_MAX_STRING_SIZE))
						{
							jsonvalues = (char **)malloc(sizeof(char *)*num_fields);
							if (!jsonvalues) {
								pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
				...
				if (i<OPH_SESSION_LAST_STATUS_SIZE) break;
				...
				for (i=0;i<OPH_USER_SESSION_LAST_STATUS_SIZE; ++i)
				{
					...
					{
						if (!key || !strncasecmp(key,OPH_OPERATOR_GET_CONFIG_PARAMETER_ALL,OPH_MAX_STRING_SIZE) || !strncasecmp(key,usparameters[i],OPH_MAX_STRING_SIZE))
						{
							jsonvalues = (char **)malloc(sizeof(char *)*num_fields);
							if (!jsonvalues) {
								pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
				...
				if (i<OPH_USER_SESSION_LAST_STATUS_SIZE) break;
			}
  ...
  if (oph_known_operator == OPH_RESUME_OPERATOR)
  {
	...
	for (i=0;i<wf->tasks[0].arguments_num;++i)
	{
		if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i],OPH_ARG_SESSION,OPH_MAX_STRING_SIZE))
		{
			if (!session)
			{
				if (!strncasecmp(wf->tasks[0].arguments_values[i],OPH_COMMON_PARAMETER_WORKING_SESSION,OPH_MAX_STRING_SIZE)) session = wf->sessionid;
				else session = wf->tasks[0].arguments_values[i];
			}
		}
		...
		else if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i],OPH_ARG_IDTYPE,OPH_MAX_STRING_SIZE))
		{
			if (id_type<0)
			{
				if (!strncasecmp(wf->tasks[0].arguments_values[i],OPH_OPERATOR_RESUME_PARAMETER_MARKER,OPH_MAX_STRING_SIZE)) id_type=1;
				else if (!strncasecmp(wf->tasks[0].arguments_values[i],OPH_OPERATOR_RESUME_PARAMETER_WORKFLOW,OPH_MAX_STRING_SIZE)) id_type=0;
				else
				{
		...
		else if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i],OPH_ARG_DOCUMENTTYPE,OPH_MAX_STRING_SIZE))
		{
			if (document_type<0)
			{
				if (!strncasecmp(wf->tasks[0].arguments_values[i],OPH_OPERATOR_RESUME_PARAMETER_REQUEST,OPH_MAX_STRING_SIZE)) document_type=1;
				else if (!strncasecmp(wf->tasks[0].arguments_values[i],OPH_OPERATOR_RESUME_PARAMETER_RESPONSE,OPH_MAX_STRING_SIZE)) document_type=0;
				else
				{
		...
		else if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i],OPH_ARG_SAVE,OPH_MAX_STRING_SIZE))
		{
			if (save<0)
			{
				if (!strncasecmp(wf->tasks[0].arguments_values[i],OPH_COMMON_YES,OPH_MAX_STRING_SIZE)) save=1;
				else if (!strncasecmp(wf->tasks[0].arguments_values[i],OPH_COMMON_NO,OPH_MAX_STRING_SIZE)) save=0;
				else
				{
					pmesg_safe(&global_flag, LOG_WARNING, __FILE__,__LINE__,"R%d: received wrong parameter '%s'\n",jobid,OPH_ARG_SAVE);
					response->error = OPH_SERVER_WRONG_PARAMETER_ERROR;
		...
		{
			if (!user) user = wf->tasks[0].arguments_values[i];
		}
	}
	
	if (id<0) id=0;
	if (id_type<0) id_type=0;
	if (document_type<0) document_type=0;
	if (level<0) level=1;
	if (save<0) save=0;

	if (level>4) document_type=2;
	if (level>3)
	{
		level=3; // Or less
		...
		if (id) wid=id; // Remember only the workflow id (wid)
		id=0;

		if (!document_type) document_type=1; // Level 4 is associated only "requests for a specific workflow". Currently, this parameter is useless
	}
	...
	if (!oph_get_arg(args,OPH_SESSION_LAST_MARKER,tmp)) last_markerid = strtol(tmp,NULL,10);
	if (!oph_get_arg(args,OPH_SESSION_LAST_WORKFLOW,tmp)) last_workflowid = strtol(tmp,NULL,10);
	oph_cleanup_args(&args);
	...
	if (!id) // Return a JSON Response with the list of oph_jobid submitted within the target session
	{
		...
		if (success)
		{
			...
			while (!success)
			{
				// Header
				  ...
				  if (oph_json_add_grid(oper_json,OPH_JSON_OBJKEY_RESUME,document_type ? (id_type ? "Command List" : "Request List") : (id_type ? "Response List" : "Workflow List"),NULL,jsonkeys,num_fields,fieldtypes,num_fields)) {
					  pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: ADD GRID error\n", jobid);
					  for (iii = 0; iii < num_fields; iii++) if (jsonkeys[iii]) free(jsonkeys[iii]);
					  if (jsonkeys) free(jsonkeys);
					  for (iii = 0; iii < num_fields; iii++) if (fieldtypes[iii]) free(fieldtypes[iii]);
		...
		if (success)
		{
			...
			if (user && !id_type) snprintf(query,OPH_MAX_STRING_SIZE, MYSQL_RETRIEVE_WORKFLOWS_OF_USER_SESSION,session,user);
			else if (document_type && id_type)
			{
				if (wid) snprintf(query,OPH_MAX_STRING_SIZE,MYSQL_RETRIEVE_SUBMISSION_STRINGS_OF_WORKFLOW,session,wid);
				else snprintf(query,OPH_MAX_STRING_SIZE,MYSQL_RETRIEVE_SUBMISSION_STRINGS_OF_SESSION,session);
			}
			else snprintf(query,OPH_MAX_STRING_SIZE,id_type ? MYSQL_RETRIEVE_JOBS_OF_SESSION : MYSQL_RETRIEVE_WORKFLOWS_OF_SESSION,session);
			...
			while (!success)
			{
				...
				for (i=0;i<list.size;++i)
				{
					if (document_type)
					{
						...
						if (id_type) submission_string = strdup(list.name[i] ? list.name[i] : "-");
						else
						{
							orig_request = document_type>1;
							if (!orig_request) snprintf(filename, OPH_MAX_STRING_SIZE, OPH_SESSION_JSON_REQUEST_FOLDER_TEMPLATE"/"OPH_SESSION_OUTPUT_EXT, oph_web_server_location, session_code, list.wid[i]);
							if (orig_request || (stat(filename,&s) && (errno==ENOENT))) snprintf(filename, OPH_MAX_STRING_SIZE, OPH_SESSION_JSON_REQUEST_FOLDER_TEMPLATE"/"OPH_SESSION_OUTPUT_MAIN, oph_web_server_location, session_code, list.wid[i]);
							...
							if (level<3)
							{
								if (!oph_workflow_load(buffer,soap->userid,&old_wf))
								{
									if (level==1)
									{
										if (old_wf->command) submission_string = strdup(old_wf->command);
					  					else if (old_wf->tasks_num==1)
										{
											if (oph_workflow_get_submitted_string(old_wf, 0, -1, 1, &submission_string)) submission_string = NULL;
										}
										else submission_string = strdup(old_wf->name);
									}
									else
									{
										...
										else if (old_wf->command) submission_string = strdup(old_wf->command);
										else submission_string = strdup(old_wf->name);
									}
									oph_workflow_free(old_wf);
								}
							}
							else submission_string = strdup(buffer);
							free(buffer);
						}
					}
					...
					jjj++;
					if (list.wid[i]) snprintf(tmp,OPH_SHORT_STRING_SIZE,"%d",list.wid[i]);
					else snprintf(tmp,OPH_SHORT_STRING_SIZE,"-");
					jsonvalues[jjj] = strdup(tmp);
					if (!jsonvalues[jjj]) {
						pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
						for (iii = 0; iii < jjj; iii++) if (jsonvalues[iii]) free(jsonvalues[iii]);
					...
					{
						jjj++;
						if (list.pid[i]) snprintf(tmp,OPH_SHORT_STRING_SIZE,"%d",list.pid[i]);
						else snprintf(tmp,OPH_SHORT_STRING_SIZE,"-");
						jsonvalues[jjj] = strdup(tmp);
						if (!jsonvalues[jjj]) {
							pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
		...
		if (success && save) response->jobid = soap_strdup(soap, session);
	...
	else // Return the intended JSON Response
	{
		...
		if (!document_type && !id_type && ((level==1) || (level==3))) // JSON Response for workflow: extract specific outputs
		{
			...
			if (oph_odb_retrieve_ids(&oDB, query, &markers, &ctime, &n))
			{
				...
				if (markers) { free(markers); markers=NULL; }
				if (ctime) { free_string_vector(ctime,n); ctime=NULL; }
				oph_odb_disconnect_from_ophidiadb(&oDB);
				oph_workflow_free(wf);
				oph_cleanup_args(&user_args);
				response->error = OPH_SERVER_IO_ERROR;
		...
		else if (document_type && id_type) // Request for specific submission strings
		{
			...
			if (!creation_date)
			{
				pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: memory allocation error\n",jobid);
				if (markers) { free(markers); markers=NULL; }
				if (creation_date) free(creation_date);
				oph_odb_free_ophidiadb_list(&list);
				oph_odb_disconnect_from_ophidiadb(&oDB);
				oph_workflow_free(wf);
				oph_cleanup_args(&user_args);
		...
		if (success)
		{
			...
			{
				if (id_type) workflow = markers[i]; else marker = markers[i];
				if (!marker || !workflow)
				{
				...
				if (!i)
				{
					...
					if (n>1) continue; // In case of more than one rows then the first is related to the parent and the others are related to children
				}
#ifdef LEVEL1
				...
				if (!document_type || !id_type)
				{
					...
					pthread_mutex_lock(&global_flag);
					item = id_type ? oph_find_marker_in_job_list(state->job_info, session, marker, &task_index, &light_task_index) : oph_find_workflow_in_job_list(state->job_info, session, workflow);

					if (item) // Found a workflow in memory
					{
						if (document_type) item = NULL; // Load the virtual JSON Request
						else // Build the virtual JSON Response
						{
							...
							if (task_index<0)
							{
								...
								snprintf(str_jobid,OPH_MAX_STRING_SIZE,"%s%s%d%s%d",item->wf->sessionid,OPH_SESSION_WORKFLOW_DELIMITER,item->wf->workflowid,OPH_SESSION_MARKER_DELIMITER,item->wf->markerid);
								...
								*error_message=0;
								...
								if (success && item) // Warning... "item" was in the shared memory, but now it is unbound from the list, so that it can be read after releasing the lock!!!
								{
									...
									while (!success)
									{
										if (oph_json_add_text_unsafe(oper_json,OPH_JSON_OBJKEY_WORKFLOW_STATUS,"Workflow Status",oph_odb_convert_status_to_str(item->wf->status)))
										{
										...
										// Header
										  ...
										  if (oph_json_add_grid_unsafe(oper_json,OPH_JSON_OBJKEY_WORKFLOW_LIST,"Workflow Task List",NULL,jsonkeys,num_fields,fieldtypes,num_fields)) {
										...
										for (i=0;i<item->wf->tasks_num;++i) ppp[i]=i;
										do // Order by markerid
										{
											make_swap=0;
											for (i=0;i<item->wf->tasks_num-max_swap;++i) if (item->wf->tasks[ppp[i]].markerid > item->wf->tasks[ppp[i+1]].markerid)
											{
												swap=ppp[i]; ppp[i]=ppp[i+1]; ppp[i+1]=swap;
												make_swap=1; max_swap++;
											}
										}
										while (make_swap);
										...
										for (i=0;i<item->wf->tasks_num;++i)
										{
											if (item->wf->tasks[i].status && (item->wf->tasks[i].status < OPH_ODB_STATUS_ABORTED)) // Discard uninitialized or aborted jobs
											{
												  ...
												  jjj=0;
												  snprintf(jsontmp,OPH_MAX_STRING_SIZE,"%s%s%d%s%d",item->wf->sessionid,OPH_SESSION_WORKFLOW_DELIMITER,item->wf->workflowid,OPH_SESSION_MARKER_DELIMITER,item->wf->tasks[i].markerid);
												  jsonvalues[jjj] = strdup(jsontmp);
												  if (!jsonvalues[jjj]) {
													  pmesg(LOG_ERROR, __FILE__, __LINE__, "%c%d: Error allocating memory\n", ttype, jobid);
													  for (iii = 0; iii < jjj; iii++) if (jsonvalues[iii]) free(jsonvalues[iii]);
								...
								if (oper_json)
								{
									...
									if (!success)
									{
										...
										else if (oph_json_to_json_string_unsafe(oper_json, &jstring)) return_code = -1;
									}
									else
									{
									...
									if (!return_code) pmesg(LOG_DEBUG, __FILE__,__LINE__, "%c%d: JSON output for workflow '%s' has been written\n", ttype, jobid, item->wf->name);
								}
								oph_json_free(oper_json);
							}
							else if ((light_task_index<0) && (item->wf->tasks[task_index].light_tasks_num)) // Massive operation
							{
								...
								snprintf(str_jobid,OPH_MAX_STRING_SIZE,"%s%s%d%s%d",item->wf->sessionid,OPH_SESSION_WORKFLOW_DELIMITER,item->wf->workflowid,OPH_SESSION_MARKER_DELIMITER,item->wf->tasks[task_index].markerid);
								snprintf(str_workflowid,OPH_SHORT_STRING_SIZE,"%d",item->wf->workflowid);
								...
								else
								{
									...
									while (!success)
									{
										if (oph_json_add_text(oper_json,OPH_JSON_OBJKEY_MASSIVE_STATUS,"Massive Operation Status",oph_odb_convert_status_to_str(item->wf->tasks[task_index].status)))
										{
										...
										{
											make_swap=0;
											for (i=0;i<item->wf->tasks_num-max_swap;++i) if (item->wf->tasks[task_index].light_tasks[ppp[i]].markerid > item->wf->tasks[task_index].light_tasks[ppp[i+1]].markerid)
											{
												swap=ppp[i]; ppp[i]=ppp[i+1]; ppp[i+1]=swap;
												make_swap=1; max_swap++;
											}
										...
										while (make_swap);
										  ...
										  for (i=0;i<item->wf->tasks[task_index].light_tasks_num;++i)
										  {
											if (item->wf->tasks[task_index].light_tasks[i].status && (item->wf->tasks[task_index].light_tasks[i].status < OPH_ODB_STATUS_ABORTED)) // Discard uninitialized or aborted jobs
											{
												  ...
												  jjj=0;
												  snprintf(jsontmp,OPH_MAX_STRING_SIZE,"%s%s%d%s%d",item->wf->sessionid,OPH_SESSION_WORKFLOW_DELIMITER,item->wf->workflowid,OPH_SESSION_MARKER_DELIMITER,item->wf->tasks[task_index].light_tasks[i].markerid);
												  jsonvalues[jjj] = strdup(jsontmp);
												  if (!jsonvalues[jjj]) {
													  pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
													  for (iii = 0; iii < jjj; iii++) if (jsonvalues[iii]) free(jsonvalues[iii]);
												  ...
												  jjj++;
												  snprintf(jsontmp,OPH_SHORT_STRING_SIZE,"%d",item->wf->tasks[task_index].light_tasks[i].markerid);
												  jsonvalues[jjj] = strdup(jsontmp);
												  if (!jsonvalues[jjj]) {
													  pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
													  for (iii = 0; iii < jjj; iii++) if (jsonvalues[iii]) free(jsonvalues[iii]);
												  ...
												  jjj++;
												  jsonvalues[jjj] = strdup(oph_odb_convert_status_to_str(item->wf->tasks[task_index].light_tasks[i].status));
												  if (!jsonvalues[jjj]) {
													  pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
													  for (iii = 0; iii < jjj; iii++) if (jsonvalues[iii]) free(jsonvalues[iii]);
													  if (jsonvalues) free(jsonvalues);
										...
										if (i<item->wf->tasks[task_index].light_tasks_num) break;
										else success=1;
									}
								}
								...
								if (oper_json)
								{
									...
									else
									{
										...
										if (!number_of_jobs && oph_json_add_text_unsafe(oper_json,OPH_JSON_OBJKEY_MASSIVE_SUMMARY,"WARNING","No job found!"))
										{
							...
							else // Single task (possible light task)
							{
								...
								snprintf(str_jobid,OPH_MAX_STRING_SIZE,"%s%s%d%s%d",item->wf->sessionid,OPH_SESSION_WORKFLOW_DELIMITER,item->wf->workflowid,OPH_SESSION_MARKER_DELIMITER,marker);
								snprintf(str_workflowid,OPH_SHORT_STRING_SIZE,"%d",item->wf->workflowid);
								...
								*error_message = 0;
								...
								else
								{
									...
									while (!success)
									{
										if (oph_json_add_text(oper_json,OPH_JSON_OBJKEY_RESUME_STATUS,"Job Status",oph_odb_convert_status_to_str(light_task_index >=0 ? item->wf->tasks[task_index].light_tasks[light_task_index].status : item->wf->tasks[task_index].status)))
										{
										    	pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "%c%d: ADD TEXT error\n", ttype, jobid);
											break;
										}
					...
					if (!item) // Check for the file
					{
						if (document_type)
						{
							...
							if (!orig_request) snprintf(filename, OPH_MAX_STRING_SIZE, OPH_SESSION_JSON_REQUEST_FOLDER_TEMPLATE"/"OPH_SESSION_OUTPUT_EXT, oph_web_server_location, session_code, workflow);
							if (orig_request || (stat(filename,&s) && (errno==ENOENT))) snprintf(filename, OPH_MAX_STRING_SIZE, OPH_SESSION_JSON_REQUEST_FOLDER_TEMPLATE"/"OPH_SESSION_OUTPUT_MAIN, oph_web_server_location, session_code, workflow);
						}
						else snprintf(filename, OPH_MAX_STRING_SIZE, OPH_SESSION_JSON_RESPONSE_FOLDER_TEMPLATE"/"OPH_SESSION_OUTPUT_MAIN, oph_web_server_location, session_code, marker);
						if (oph_get_result_from_file(filename,&jstring) || !jstring)
						{
							pmesg_safe(&global_flag, LOG_WARNING, __FILE__,__LINE__,"R%d: unable to load JSON %s '%s'\n",jobid,document_type?"Request":"Response",filename);
							if (markers) { free(markers); markers=NULL; }
							if (ctime) { free_string_vector(ctime,n); ctime=NULL; }
							response->error = OPH_SERVER_NO_RESPONSE;
							oph_workflow_free(wf);
				...
				if (document_type)
				{
					if (id_type) jstring = submission_string;
			...
			if (!document_type) jstring = buffer;
			else free(buffer);
		}
  ...
  if (max_cores) max_cores = OPH_DEFAULT_USER_MAX_CORES;
  else max_cores = strtol(tmp,NULL,10);

  pmesg_safe(&global_flag, LOG_DEBUG, __FILE__,__LINE__, "R%d: check for %s\n",jobid,OPH_USER_MAX_HOSTS);
  int max_hosts = oph_get_arg(user_args,OPH_USER_MAX_HOSTS,tmp);
  if (max_hosts) max_hosts = OPH_DEFAULT_USER_MAX_HOSTS;
  else max_hosts = strtol(tmp,NULL,10);
  ...
  pthread_mutex_lock(&global_flag);
  if ((result = oph_generate_oph_jobid(state, 'R', jobid, wf, &num_sessions, max_sessions, timeout_value, &(wf->markerid), str_markerid, &(wf->workflowid), str_workflowid, oph_jobid, wf->markerid)))
  {
	pthread_mutex_unlock(&global_flag);
	oph_cleanup_args(&user_args);
	oph_workflow_free(wf);
  ...
  else pmesg_safe(&global_flag, LOG_WARNING, __FILE__,__LINE__, "R%d: unable to save the request\n", jobid);

  char* submission_string = NULL;
  if (wf->command) submission_string = strdup(wf->command);
  else if (wf->tasks_num==1)
  {
	if (oph_workflow_get_submitted_string(wf, 0, -1, 0, &submission_string)) submission_string = NULL;
  }
  else submission_string = strdup(wf->name);
  if (!oph_session_report_append_command(session_code,wf->workflowid,wf->markerid,wf->username,submission_string ? submission_string : request)) pmesg(LOG_DEBUG, __FILE__,__LINE__, "R%d: command added to session report\n", jobid);
  if (submission_string) free(submission_string);
	...
	if (wf->parallel_mode)
	{
		...
		else pmesg_safe(&global_flag, LOG_WARNING, __FILE__,__LINE__, "Unable to save the extended JSON Request\n");
	...
	if ((result = oph_wf_list_append(state->job_info, wf)))
	{
		...
		else wf->status = OPH_ODB_STATUS_UNKNOWN; // Used only for queue management, real status is OPH_ODB_STATUS_PENDING
	}
	...
	if (mode_type == OPH_MODE_SYNC)
	{
		...
		if (wf->response && (wf->status == OPH_ODB_STATUS_CLOSED))
		{
			...
			if (wf->cube)
			{
				...
				{
					if (oph_known_operator == OPH_MANAGE_SESSION_OPERATOR) skip = 1;
					else
					{
		...
		else
		{
			...
			oph_workflow_free(wf);
			if (!response->error) response->error = OPH_SERVER_SYSTEM_ERROR;
			return SOAP_OK;
		}
	}
