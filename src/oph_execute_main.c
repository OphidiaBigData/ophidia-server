	...
	*_new_token = 0;
	...
	if (oph_known_operator == OPH_SERVICE_OPERATOR) {
		...
		for (i = 0; i < wf->tasks[0].arguments_num; ++i) {
			...
			} else if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i], OPH_SERVER_CONF_SERVER_FARM_SIZE, OPH_MAX_STRING_SIZE)) {
				value = wf->tasks[0].arguments_values[i];
				if (value && strlen(value)) {
					new_value = (int) strtol(value, NULL, 10);
					if (new_value < 0) {
						pmesg(LOG_WARNING, __FILE__, __LINE__, "R%d: received wrong farm size '%s'\n", jobid, value);
						pthread_mutex_unlock(&global_flag);
						response->error = OPH_SERVER_WRONG_PARAMETER_ERROR;
						oph_workflow_free(wf);
						return SOAP_OK;
					}
					oph_server_farm_size = new_value;
				}
			} else if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i], OPH_SERVER_CONF_QUEUE_SIZE, OPH_MAX_STRING_SIZE)) {
				value = wf->tasks[0].arguments_values[i];
				if (value && strlen(value)) {
					new_value = (int) strtol(value, NULL, 10);
					if (new_value < 0) {
					...
					oph_server_queue_size = new_value;
				}
			} else if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i], OPH_SERVER_CONF_TASK_LIMIT, OPH_MAX_STRING_SIZE)) {
				value = wf->tasks[0].arguments_values[i];
				if (value && strlen(value)) {
					new_value = (int) strtol(value, NULL, 10);
					if (new_value < 0) {
					...
					oph_server_task_limit = new_value;
				}
			} else if (wf->tasks[0].arguments_keys[i] && !strncasecmp(wf->tasks[0].arguments_keys[i], OPH_SERVER_CONF_CORE_LIMIT, OPH_MAX_STRING_SIZE)) {
				value = wf->tasks[0].arguments_values[i];
				if (value && strlen(value)) {
					new_value = (int) strtol(value, NULL, 10);
					if (new_value < 0) {
		...
		*tmp = 0;
		...
		if (success && (new_value >= 0)) {

			success = 0;
			snprintf(error_message, OPH_MAX_STRING_SIZE, "Failure in setting JSON data!");

			int num_fields = 2, ii, jj, iii, jjj = 0;
			...
			while (!success) {
				...
				if (oph_json_add_grid(oper_json, OPH_JSON_OBJKEY_SERVICE_PARAMETERS, "Parameter List", NULL, jsonkeys, num_fields, fieldtypes, num_fields)) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: ADD GRID error\n", jobid);
					for (iii = 0; iii < num_fields; iii++)
						if (jsonkeys[iii])
							free(jsonkeys[iii]);
				...
				jjj = 0;
				jsonvalues[jjj] = strdup(OPH_SERVER_CONF_SERVER_FARM_SIZE);
				if (!jsonvalues[jjj]) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
					for (iii = 0; iii < jjj; iii++)
						if (jsonvalues[iii])
				...
				jjj++;
				snprintf(jsontmp, OPH_SHORT_STRING_SIZE, "%d", oph_server_farm_size);
				jsonvalues[jjj] = strdup(jsontmp);
				if (!jsonvalues[jjj]) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
					for (iii = 0; iii < jjj; iii++)
				...
				for (iii = 0; iii < num_fields; iii++)
					if (jsonvalues[iii])
						free(jsonvalues[iii]);
				jjj = 0;
				jsonvalues[jjj] = strdup(OPH_SERVER_CONF_QUEUE_SIZE);
				if (!jsonvalues[jjj]) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
					for (iii = 0; iii < jjj; iii++)
						if (jsonvalues[iii])
				...
				jjj++;
				snprintf(jsontmp, OPH_SHORT_STRING_SIZE, "%d", oph_server_queue_size);
				jsonvalues[jjj] = strdup(jsontmp);
				if (!jsonvalues[jjj]) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
					for (iii = 0; iii < jjj; iii++)
				...
				for (iii = 0; iii < num_fields; iii++)
					if (jsonvalues[iii])
						free(jsonvalues[iii]);
				jjj = 0;
				jsonvalues[jjj] = strdup(OPH_SERVER_CONF_TASK_LIMIT);
				if (!jsonvalues[jjj]) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
					for (iii = 0; iii < jjj; iii++)
						if (jsonvalues[iii])
				...
				jjj++;
				snprintf(jsontmp, OPH_SHORT_STRING_SIZE, "%d", oph_server_task_limit);
				jsonvalues[jjj] = strdup(jsontmp);
				if (!jsonvalues[jjj]) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
					for (iii = 0; iii < jjj; iii++)
				...
				for (iii = 0; iii < num_fields; iii++)
					if (jsonvalues[iii])
						free(jsonvalues[iii]);
				jjj = 0;
				jsonvalues[jjj] = strdup(OPH_SERVER_CONF_CORE_LIMIT);
				if (!jsonvalues[jjj]) {
					pmesg_safe(&global_flag, LOG_ERROR, __FILE__, __LINE__, "R%d: Error allocating memory\n", jobid);
					for (iii = 0; iii < jjj; iii++)
						if (jsonvalues[iii])
					...
					if (!document_type || !id_type) {
						...
						} else if (item)	// Found a workflow in memory
						{
							...
							if (oph_get_result_from_file_unsafe(filename, &jstring) || !jstring) {
								...
								char ttype = 'R', session_code[OPH_MAX_STRING_SIZE], is_compact = 1, is_extended = 1;
								int make_swap, swap, max_swap = 1;

								switch (item->wf->output_format) {
									case 3:	// CLASSIC
										is_compact = 0;
										is_extended = 0;
										break;
												...
												if (item->wf->tasks[i].status && item->wf->tasks[i].markerid
													...
													if (is_extended) {
														...
														if (!task->outputs_values)
															...
															if (task->outputs_values[task->outputs_file])
																jsonvalues[jjj] = strdup(task->outputs_values[task->outputs_file]);
															else
																jsonvalues[jjj] = strdup("");
														} else {
															for (iii = 0; iii < task->outputs_num; ++iii)
																if (!strncmp
																	...
																	break;
															if ((iii < task->outputs_num) && task->outputs_values[iii])
																jsonvalues[jjj] = strdup(task->outputs_values[iii]);
															else
																jsonvalues[jjj] = strdup("");
														}
														if (!jsonvalues[jjj]) {
														...
														jjj++;
														jsonvalues[jjj] = oph_workflow_arguments_of_task(task);
														if (!jsonvalues[jjj]) {
															pmesg(LOG_ERROR, __FILE__, __LINE__, "%c%d: Error allocating memory\n", ttype,
															      jobid);
															for (iii = 0; iii < jjj; iii++)
												...
												if (item->wf->tasks[task_index].light_tasks[i].status
													...
													if (is_extended) {
														...
														if (strlen(jsonvalues[jjj]))
															oph_workflow_var_substitute(item->wf, task_index, i, jsonvalues + jjj, NULL,
																		    NULL);
														// Remove reference in case no new data is produced
														if (strlen(jsonvalues[jjj - 1]) && strlen(jsonvalues[jjj])
														    && !strcmp(jsonvalues[jjj - 1], jsonvalues[jjj])) {
														...
														jjj++;
														if (item->wf->tasks[task_index].begin_time)
															jsonvalues[jjj] = strdup(item->wf->tasks[task_index].begin_time);
														else
															jsonvalues[jjj] = strdup("");
														if (!jsonvalues[jjj]) {
															pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
														...
														jjj++;
														if (item->wf->tasks[task_index].light_tasks[i].end_time)
															jsonvalues[jjj] = strdup(item->wf->tasks[task_index].light_tasks[i].end_time);
														else
															jsonvalues[jjj] = strdup("");
														if (!jsonvalues[jjj]) {
															pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
														...
														jjj++;
														jsonvalues[jjj] =
														    oph_workflow_arguments_of_light_task(item->wf->tasks[task_index].light_tasks + i);
														if (!jsonvalues[jjj]) {
															pmesg(LOG_ERROR, __FILE__, __LINE__, "N%d: Error allocating memory\n", jobid);
															for (iii = 0; iii < jjj; iii++)
																if (jsonvalues[iii])
